...
- **All 15 workspace packages typecheck clean** under `strict`, `noUncheckedIndexedAccess`, `exactOptionalPropertyTypes`.
- **991 unit tests passing** across the repo; 22 Playwright e2e on Chromium / Firefox / WebKit.
...
- **Live L5 sensor stream → dashboard** works: `tools/carla/vsbs_carla/scripts/run_chaos_demo.py` (or the real CARLA bridge) POSTs 10 Hz frames + 21 perception events to `/v1/autonomy/:id/{telemetry,events}/ingest`; dashboard subscribes via SSE; 17 sections render including BEV occupancy, 96-cell HV heat-map, OOD/SOTIF, V2X bus.
- **Full repo builds**: libs via tsc, API via `bun build`, web via `next build` (Next.js 16.2.3).
...
| Understand the live L5 telemetry hub | [`apps/api/src/adapters/autonomy/live-hub.ts`](apps/api/src/adapters/autonomy/live-hub.ts) (Zod schema + ring-buffered pub/sub) + [`apps/api/src/adapters/autonomy/synthetic-frame.ts`](apps/api/src/adapters/autonomy/synthetic-frame.ts) (deterministic fallback) |
| Render the L5 sensor suite on the dashboard | [`apps/web/src/components/autonomy/SensorSuite.tsx`](apps/web/src/components/autonomy/SensorSuite.tsx) (12 sections: cameras, BEV, GNSS, dynamics, powertrain cells, compute, V2X, SOTIF, cabin, env) + [`PerceptionEventLog.tsx`](apps/web/src/components/autonomy/PerceptionEventLog.tsx) |
| Drive the dashboard with no GPU | [`tools/carla/vsbs_carla/scripts/run_chaos_demo.py`](tools/carla/vsbs_carla/scripts/run_chaos_demo.py) — wire-identical to the live CARLA bridge, 10 Hz frames + 21-event scenario |
...
- **2026-05-01 (Web UI fix-up + L5 sensor stream)** — Two waves in one session. **Wave A** drove Playwright against the running web app and fixed every shipping defect: strict CSP `style-src` was blocking inline `style={...}` JSX props (root cause for "only one image loads"); Tailwind 4 needed `text-[length:var(--text-…)]` not `text-[var(--text-…)]` across 211 callsites (root cause for collapsed typography hierarchy); `/api/_/csp-report` 404'd, `/favicon.ico` 404'd, `/api/proxy/web-vitals` hit the wrong path, CameraTile clock and FALLBACK_GRANT.issuedAt caused hydration mismatch, useTelemetryStream tried WS through a HTTP-only proxy. Hero PNGs converted PNG→WebP (66 MB → 1.4 MB). Witness at [`docs/verification/2026-05-01-web-ui-fix.md`](docs/verification/2026-05-01-web-ui-fix.md). **Wave B** delivered an L5 sensor stream → dashboard pipeline. New API surface: in-memory `LiveAutonomyHub` keyed by booking id with ring-buffered telemetry + perception event channels; `POST /v1/autonomy/:id/telemetry/ingest` + `POST /:id/events/ingest` + `GET /:id/telemetry/sse` + `GET /:id/events/sse`; comprehensive `LiveTelemetryFrameSchema` covering 8 surround cameras, 4× 4D imaging radars, solid-state LiDAR, LWIR thermal, 8-mic audio array, multi-constellation GNSS+RTK, 9-DoF IMU, per-corner wheels, motors + 96-cell HV pack with isolation + 3 coolant loops, AURIX lockstep + HSM, 5G NR-V2X + MEC + HD-map, V2X bus (BSM/SPaT/MAP/CAM/DENM), ODD compliance + Mahalanobis OOD score + UNECE R157 ladder + MRM + capability budget, DMS + cabin air quality, environment, software versions; `synthetic-frame.ts` fallback fills every channel with research-grade values. New web components: `SensorSuite.tsx` renders 12 dense sections (~840 lines) with BEV occupancy mini-map + class-coded tracks + risk halos + 96-cell heat-map; `PerceptionEventLog.tsx` streams the events SSE. New CARLA bridge integration: `VsbsApi.autonomy_telemetry()` + `.autonomy_event()` helpers in [`tools/carla/vsbs_carla/api.py`](tools/carla/vsbs_carla/api.py); standalone GPU-free [`run_chaos_demo.py`](tools/carla/vsbs_carla/scripts/run_chaos_demo.py) drives a 5-minute scripted chaos scenario (red light → pedestrian dart-out → R157 rung 1 → fault progression → OOD over threshold → R157 rung 2 → MRM lateral-creep → Mercedes-Bosch IPP handshake). Path-aware rate limiter on `apps/api/src/server.ts` so 10 Hz ingest doesn't trip the global cap. Live CARLA on this 2 GiB-VRAM laptop hit `VK_ERROR_DEVICE_LOST`; the chaos driver is wire-identical so the dashboard runs end-to-end on this hardware. All 15 packages typecheck clean. Witness extended at the same `2026-05-01-web-ui-fix.md`.
