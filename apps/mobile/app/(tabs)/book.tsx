...
import { ApiError, type Booking, type BookingCreate, apiClient } from "@/lib/api";
	...
	return (
		...
		>
			<Text
				style={{
					...typography.label,
					color: selected ? palette.accentOn : palette.onSurface,
				}}
			>
				{label}
			</Text>
...
function severityFromSelfReport(
	canDrive: CanDriveSafely | "",
	redFlags: string[],
): {
	severity: Severity;
	rationale: string;
	...
	return {
		severity: "green",
		rationale: "Self-report indicates safe to drive.",
		triggered: [],
	};
}
...
export default function BookScreen() {
	...
	const [issue, setIssue] = useState<IssueDraft>({
		symptoms: "",
		canDriveSafely: "",
		redFlags: [],
	});
	...
	const safety = useMemo(
		() => severityFromSelfReport(issue.canDriveSafely, issue.redFlags),
		[issue],
	);
	...
	function toggleRedFlag(rf: string) {
		setIssue((prev) => ({
			...prev,
			redFlags: prev.redFlags.includes(rf)
				? prev.redFlags.filter((x) => x !== rf)
				: [...prev.redFlags, rf],
		}));
	}
		...
		<Screen>
			...
			{step === 2 ? (
				<Card>
					...
					<TextField
						label="Make"
						value={vehicle.make}
						onChangeText={(s) => setVehicle((v) => ({ ...v, make: s }))}
					/>
					<TextField
						label="Model"
						value={vehicle.model}
						onChangeText={(s) => setVehicle((v) => ({ ...v, model: s }))}
					/>
					<TextField
						label="Year"
						value={vehicle.year}
			...
			{step === 3 ? (
				<Card>
					...
					/>
					<Text style={{ ...typography.label, color: palette.onSurface }}>
					...
					<View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.s }}>
						{RED_FLAGS.map((rf) => (
							<Chip
								key={rf}
								label={rf}
								selected={issue.redFlags.includes(rf)}
								onPress={() => toggleRedFlag(rf)}
							/>
						))}
					</View>
					<Button
			...
			{step === 4 ? (
				<Card>
					...
					<Banner
						...
						variant={
							safety.severity === "red" ? "danger" : safety.severity === "amber" ? "warn" : "good"
						}
					/>
					<Button
