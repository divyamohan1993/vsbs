...
export async function registerPasskey(opts: {
	...
	const begin = await apiClient.request(
	...
	const authenticatorSelection: NonNullable<
		Parameters<typeof Passkey.create>[0]["authenticatorSelection"]
	> = {
 ...
 */
export async function assertOverChallenge(
	challengeB64u: string,
	rpId: string,
): Promise<AssertionForGrant> {
	const result = await Passkey.get({
