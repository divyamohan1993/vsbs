		...
		>
			...
			<div className="overflow-x-auto">
				<table
					...
					role="grid"
					aria-label={`${selectedSc} utilisation, day x hour`}
				>
					<caption className="sr-only">
						Capacity heat map. Rows are days of the week, columns are hours of the day, cell colour
						shows utilisation.
					</caption>
					<thead>
						<tr>
						...
						{DAYS.map((day, di) => (
							<tr key={day}>
								...
								{grid.hours.map((h) => {
									...
									if (!cell) {
										return (
											<td key={h} className="px-1 py-1 text-center text-muted">
												·
											</td>
										);
									}
			...
			<p className="text-muted mt-3 text-xs" aria-label={labels.scaleLabel}>
				<span className="mr-3 inline-flex items-center gap-1">
					<span
						...
						style={{ backgroundColor: "oklch(74% 0.16 155)" }}
					/>
					{labels.low}
				</span>
				<span className="mr-3 inline-flex items-center gap-1">
					<span
						className="inline-block h-3 w-3 rounded"
						style={{ backgroundColor: "oklch(80% 0.15 90)" }}
					/>
					{labels.medium}
				</span>
				<span className="inline-flex items-center gap-1">
					<span
						className="inline-block h-3 w-3 rounded"
						style={{ backgroundColor: "oklch(66% 0.22 25)" }}
					/>
					{labels.high}
				</span>
			</p>
			{selectedCell ? (
				<div
					role="region"
					aria-live="polite"
					className="mt-4 rounded-md border border-[var(--color-border)] bg-surface-3 p-3 text-sm"
				>
