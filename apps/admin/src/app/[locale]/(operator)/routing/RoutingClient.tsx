...
export function RoutingClient({ labels }: { labels: Labels }) {
	...
	const columns: Column<Route>[] = [
		...
		},
		{
			...
			header: "Technician",
			sortable: true,
			sortBy: (r) => r.technicianId,
			cell: (r) => r.technicianId,
		},
		{
			id: "sc",
			header: "SC",
		...
		},
		{
			id: "pickups",
			header: "Pickups",
			cell: (r) => <span className="text-muted font-mono text-xs">{r.pickups.join(", ")}</span>,
		},
		{
			id: "current",
	...
	return (
		<div className="space-y-4">
			<Card
				title="Active routes"
				actions={<Button onClick={() => void rerun()}>{labels.rerun}</Button>}
			>
				{error ? (
