...
export function AuditListClient({ labels, locale }: { labels: Labels; locale: string }) {
	...
	const columns: Column<Grant>[] = [
		...
		},
		{
			...
			header: "Vehicle",
			sortable: true,
			sortBy: (r) => r.vehicleId,
			cell: (r) => <code className="font-mono text-xs">{r.vehicleId}</code>,
		},
		{
			id: "owner",
			header: "Owner",
			sortable: true,
			sortBy: (r) => r.ownerId,
			cell: (r) => <code className="font-mono text-xs">{r.ownerId}</code>,
		},
		{
			id: "tier",
			header: "Tier",
		...
		},
		{
			id: "scopes",
			header: "Scopes",
			cell: (r) => <span className="text-muted text-xs">{r.scopes.join(", ")}</span>,
		},
		{
			id: "status",
			header: "Status",
		...
		},
		{
			...
			header: "Merkle idx",
			sortable: true,
			sortBy: (r) => r.merkleIndex,
			align: "right",
			cell: (r) => r.merkleIndex,
		},
		{
			id: "root",
			header: "Root idx",
			sortable: true,
			sortBy: (r) => r.rootIndex,
			align: "right",
			cell: (r) => r.rootIndex,
		},
	];
