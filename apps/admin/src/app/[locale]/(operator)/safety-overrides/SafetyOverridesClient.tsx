	...
	return (
		...
		>
			...
			<ol className="space-y-2">
				{rows.length === 0 ? <li className="text-muted">No safety overrides match.</li> : null}
				{rows.map((r) => {
						...
						<li key={r.id} className="rounded-md border border-[var(--color-border)] bg-surface-2">
							...
							>
								<span className="flex flex-wrap items-center gap-2">
									...
									<StatusPill
										tone={
											r.decision === "tow" ? "danger" : r.decision === "delay" ? "warn" : "info"
										}
									>
										{r.decision}
									</StatusPill>
							...
							{isOpen ? (
								<div
									id={`so-${r.id}`}
									className="border-t border-[var(--color-border)] px-4 py-3 text-sm"
								>
