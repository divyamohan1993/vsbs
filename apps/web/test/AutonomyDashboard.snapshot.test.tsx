...
describe("AutonomyDashboard snapshot", () => {
	it("renders the dashboard skeleton with command-grant fallback", () => {
		...
		expect(
			container.querySelector("section[aria-labelledby='cameras-heading']"),
		).not.toBeNull();
		expect(
			container.querySelector("section[aria-labelledby='sensors-heading']"),
		).not.toBeNull();
		expect(
			container.querySelector("section[aria-labelledby='phm-heading']"),
		).not.toBeNull();
