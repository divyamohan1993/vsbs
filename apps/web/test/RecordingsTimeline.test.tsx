...
describe("RecordingsTimeline", () => {
	...
	it("renders events in the supplied order with category and title", () => {
		...
		expect(
			within(items[1]!).getByText("CARLA simulator handshake"),
		).toBeInTheDocument();
		expect(within(items[1]!).getByText("carla")).toBeInTheDocument();
		expect(
			within(items[1]!).getByText("town04 · 18 actors"),
		).toBeInTheDocument();
		expect(
			within(items[2]!).getByText("Pedestrian dart-out"),
		).toBeInTheDocument();
		expect(
			within(items[3]!).getByText("Encoding composite"),
		).toBeInTheDocument();
	});
