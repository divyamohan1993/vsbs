...
import { type CapturedPhoto, uploadPhoto } from "../src/lib/photo";
...
describe("uploadPhoto enforces on-device redaction", () => {
	...
	it("rejects upload when redactionSummary is missing", async () => {
		...
		await expect(uploadPhoto(photo, "intake-1", "dashcam")).rejects.toThrow(
			/redaction has not run/i,
		);
	});
	...
	it("submits the redaction summary as a multipart field when ok=true", async () => {
		...
		const parsed = JSON.parse(redaction as string) as {
			faces: number;
			plates: number;
		};
