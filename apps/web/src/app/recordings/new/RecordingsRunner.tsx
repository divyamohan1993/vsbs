...
function RecordingsRunnerInner(): React.JSX.Element {
	...
	useEffect(() => {
		...
		// Capture restored into a const the inner async closure can see is non-null.
		const session = restored;
		async function rehydrate(): Promise<void> {
			try {
				const res = await fetch(`/api/proxy/recordings/${encodeURIComponent(session.id)}`, {
					method: "GET",
				});
				if (cancelled) return;
				if (res.status === 404) {
			...
			{download ? (
				<DownloadCard
					...
					onCopyLink={() =>
						toast.push({
							title: "Copied",
							description: "Share link is on your clipboard.",
						})
					}
					onCopyError={(msg) =>
						toast.push({
			...
			<div className="flex flex-col gap-8">
				<div className="flex flex-col gap-3">
					...
					<p className="text-[length:var(--text-body)] text-pearl-muted leading-[1.6]">
						One click captures the dashboard, the perception event stream, and the chaos scenario
						into a 4K, 60 fps file. CARLA is used when a GPU is available; the chaos driver fills in
						otherwise. Either way the wire shape is identical.
					</p>
				</div>
