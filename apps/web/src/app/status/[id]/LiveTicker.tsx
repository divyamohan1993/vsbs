...
type StepKey = "accepted" | "assigned" | "pickup" | "drivingToCentre" | "atBay" | "ready";
...
const STEPS: StepDef[] = [
	...
	{
		key: "drivingToCentre",
		match: ["en route", "en route to service centre", "driving"],
	},
...
function StatusHeader({
	...
	return (
		...
		>
			<AmbientGlow
				tone="copper"
				className="!inset-[-30%_-20%_auto_auto] !w-[60%] !h-[80%] opacity-50"
			/>
			<AmbientGlow
				tone="sky"
				className="!inset-[auto_auto_-40%_-20%] !w-[60%] !h-[60%] opacity-40"
			/>
			<div className="relative z-10 flex flex-col gap-8 px-8 py-9 md:flex-row md:items-end md:justify-between md:gap-12 md:px-12">
				<div className="flex flex-col gap-3">
					...
					<div className="flex flex-wrap items-center gap-2 text-[length:var(--text-control)] text-pearl-muted">
						...
						<span aria-hidden="true" className="text-pearl-faint">
							·
						</span>
						<span>
			...
			<div className="relative z-10 h-[180px] w-full md:h-[220px]">
				...
				<span
					...
					style={{
						...
						height: 8,
						borderRadius: 999,
						background: "var(--color-pearl-muted)",
					}}
				/>
				<span
					aria-hidden="true"
...
function Timeline({
	...
	return (
		<ol aria-label={ariaLabel} className="flex flex-col gap-3">
			{rows.map((row) => (
				<li key={row.key}>
					<TimelineRow
						row={row}
...
function TimelineRow({
	...
	return (
		<GlassPanel
			...
			className={cn("relative isolate overflow-hidden !py-5", isCurrent && "luxe-timeline-current")}
		>
			{isCurrent ? (
				<AmbientGlow
					tone="copper"
					className="!inset-[auto_-40%_-60%_auto] !w-[80%] !h-[160%] opacity-30"
				/>
			) : null}
	...
	if (state === "done") {
		return (
			<span
				...
				style={{
					background: "radial-gradient(circle, rgba(31,143,102,0.95) 0%, rgba(21,105,74,1) 70%)",
	...
	return (
		<div className="flex flex-col gap-6">
			<GlassPanel className="flex flex-col gap-6">
				...
				<KPIBlock label={etaLabel} value={etaValue} unit={etaUnit} status="ok" size="md" />
			</GlassPanel>
			{message ? (
