...
import { z } from "zod";
import { AmbientGlow, GlassPanel, GoldSeal, KPIBlock, SpecLabel } from "../../components/luxe";
...
const E164 = z.string().regex(/^\+[1-9]\d{6,14}$/, { message: "Use +<country><number>" });
...
type CanDrive = "yes-confidently" | "yes-cautiously" | "unsure" | "no" | "already-stranded";
...
const CANONICAL_NO_SAFETY_CERT = "I cannot certify safety; please consult a qualified mechanic.";
...
function ProgressHairline({
	...
	return (
		<div className="flex flex-col gap-3">
			<SpecLabel>{t(labelKey as Parameters<typeof t>[0])}</SpecLabel>
			<ol aria-label={t("book.stepper.label")} className="grid grid-cols-4 gap-2">
								...
								style={{
									...
									backgroundColor: active
										...
										: done
											...
											: "transparent",
									boxShadow: active ? "0 0 12px rgba(79,183,255,0.55)" : "none",
								}}
							/>
						</li>
					);
...
function StageWrap({
	...
	const transform = reduced ? "none" : phase === "in" ? "translateY(0)" : "translateY(12px)";
			...
			{form.challengeId === null ? (
				<GlassPanel className="space-y-5">
					...
					<div className="luxe-input flex min-h-[56px] items-stretch overflow-hidden rounded-[var(--radius-md)]">
						...
						<input
							...
							onChange={(e) => patch({ phoneLocal: e.target.value.replace(/[^0-9 ]/g, "") })}
				...
				<GlassPanel variant="elevated" className="space-y-6">
					<div className="space-y-2">
						...
						<p className="text-[length:var(--text-caption)] text-pearl-soft">
							{t("book.step1.otpHint", {
								phone: `${form.countryDial} ${form.phoneLocal}`,
							})}
						</p>
					</div>

					<OtpRow value={form.otp} onChange={(otp) => patch({ otp })} onComplete={onOtpVerify} />
					{errors.otp ? (
						<p
							role="alert"
							className="text-[length:var(--text-caption)] text-[var(--color-crimson)]"
				...
				<div className="grid gap-5 md:grid-cols-2">
					<div className="space-y-2">
						...
						<Input
							id="plate"
							value={form.plate}
							onChange={(e) => patch({ plate: e.target.value.toUpperCase().slice(0, 12) })}
			...
			<GlassPanel className="space-y-6">
				...
				<fieldset className="space-y-3">
					...
					<div className="grid gap-2 md:grid-cols-2">
						{(
							["yes-confidently", "yes-cautiously", "unsure", "no", "already-stranded"] as const
						).map((v) => {
...
function StepFourReview({
	...
	const symptomsLine =
		form.symptoms.length > 240 && !expanded ? `${form.symptoms.slice(0, 240)}…` : form.symptoms;
		...
		<article className="space-y-8">
			...
			<GlassPanel variant="elevated" className="space-y-7">
				...
				>
					...
					{form.rationale ? (
						<p className="text-[length:var(--text-caption)] text-pearl-soft">{form.rationale}</p>
					) : null}
				</ReviewRow>
...
function StepFiveConfirm({
	...
	return (
		<article className="relative isolate overflow-hidden rounded-[var(--radius-xl)] px-2 py-2">
			<div
				aria-hidden="true"
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				...
				<DialogDescription>
					We pull the VIN, odometer, and fault codes directly from your OEM account. Only Mercedes
					me is live today.
				</DialogDescription>
