	...
	useEffect(() => {
		...
		async function read(): Promise<void> {
			if (typeof navigator !== "undefined" && "serviceWorker" in navigator) {
				...
				if (!cancelled) {
					setSwVersion(
						reg?.active?.scriptURL ? new URL(reg.active.scriptURL).pathname : labels.swUnavailable,
					);
				}
			}
			try {
				...
				const last = (await mod.getMeta("lastOnline").catch(() => null)) as number | null;
	...
	return (
		<div className="relative isolate mx-auto flex min-h-[640px] w-full max-w-[820px] items-center justify-center px-6 py-[80px]">
			...
			>
				...
				<AmbientGlow
					tone="sky"
					className="!inset-[-30%_auto_auto_-20%] !w-[80%] !h-[80%] opacity-50"
				/>
				<AmbientGlow
					tone="copper"
					className="!inset-[auto_-20%_-30%_auto] !w-[60%] !h-[60%] opacity-40"
				/>
			</div>
				...
				<div className="flex flex-col gap-7">
										...
										background:
											dot === "sky"
												...
												: dot === "copper"
													? "var(--color-copper)"
													: "var(--color-emerald)",
										boxShadow: dot === "copper" ? "0 0 8px rgba(201,163,106,0.55)" : "none",
									}}
								/>
								<span className="text-[length:var(--text-control)] leading-[1.55] text-pearl">
									{line}
								</span>
							</li>
						))}
					...
					<div className="flex justify-center pt-2">
						<Button
							onClick={onTryAgain}
							loading={tryingAgain}
							loadingText={labels.tryingAgain}
							size="lg"
						>
							{labels.tryAgain}
						</Button>
