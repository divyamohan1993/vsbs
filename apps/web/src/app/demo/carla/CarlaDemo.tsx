...
async function readLatest(vehicleId: string): Promise<Record<string, SensorBucketEntry>> {
	...
	const body = (await res.json()) as {
		data: Record<string, SensorBucketEntry>;
	};
	return body.data ?? {};
}
...
export function CarlaDemo({ initialVehicleId }: { initialVehicleId: string }): React.JSX.Element {
	...
	const tiles = useMemo(() => {
		...
		return {
			...
			brakePad: typeof brakeP.brake_pad_pct === "number" ? brakeP.brake_pad_pct.toFixed(1) : "--",
			coolant: typeof obd.coolant_c === "number" ? Number(obd.coolant_c).toFixed(1) : "--",
			hvSoc: typeof bms.soc_pct === "number" ? bms.soc_pct.toFixed(1) : "--",
			cellDelta: typeof bms.cell_delta_mv === "number" ? bms.cell_delta_mv.toFixed(0) : "--",
			tpmsLine: tpms ? JSON.stringify(tpms.value) : "--",
		};
	}, [latest]);

	const brakePadValue = Number.parseFloat(tiles.brakePad);
	const coolantValue = Number.parseFloat(tiles.coolant);
	const cellValue = Number.parseFloat(tiles.cellDelta);

	return (
		<div className="space-y-8">
			<header className="space-y-2">
				<p className="text-muted text-xs uppercase tracking-[0.2em]">{t("demoCarla.eyebrow")}</p>
			...
			<fieldset className="rounded-[var(--radius-card)] border border-muted/30 p-6 space-y-4">
				<legend className="font-display text-xl font-semibold">{t("demoCarla.controls")}</legend>
			...
			<section aria-labelledby="telemetry-h" className="space-y-3">
				...
				<div className="grid gap-3 md:grid-cols-3">
					...
					<TelemetryTile
						...
						unit="%"
						tone={
							!Number.isNaN(brakePadValue) && brakePadValue < 25
								? "alert"
								: !Number.isNaN(brakePadValue) && brakePadValue < 40
									? "watch"
									: "neutral"
						}
					/>
					<TelemetryTile
						...
						unit="°C"
						tone={
							!Number.isNaN(coolantValue) && coolantValue >= 110
								? "alert"
								: !Number.isNaN(coolantValue) && coolantValue >= 100
									? "watch"
									: "neutral"
						}
					/>
			...
			<section
				aria-labelledby="grant-h"
				className="space-y-2 rounded-[var(--radius-card)] border border-muted/30 p-6"
			>
