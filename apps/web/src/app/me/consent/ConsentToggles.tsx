...
import { GlassPanel, GoldSeal, KPIBlock, SpecLabel } from "../../../components/luxe";
import {
	Button,
	Dialog,
	DialogContent,
...
function ConsentDashboardInner({ purposes }: { purposes: Purpose[] }): React.JSX.Element {
	...
	const refresh = useCallback(async (): Promise<void> => {
		setError(null);
		try {
			const res = await fetch("/api/proxy/me/consent", {
	...
	return (
		<div className="space-y-10" aria-busy={pending || undefined}>
			...
			{snapshot && snapshot.needsReConsent.length > 0 ? (
				<GlassPanel role="status" className="!p-5" style={{ borderColor: "var(--color-amber)" }}>
					<p className="text-pearl">
						{t("consent.staleBanner", {
							count: snapshot.needsReConsent.length,
						})}
					</p>
				</GlassPanel>
			) : null}
				...
				<DialogContent>
					...
					{noticeFor ? (
						<>
							...
							<DialogDescription>{t(`consent.noticeBody.${noticeFor}`)}</DialogDescription>
			...
			<Dialog open={consequenceFor !== null} onOpenChange={(o) => !o && setConsequenceFor(null)}>
				<DialogContent>
			...
			>
				<DialogContent className="!border-2" style={{ borderColor: "var(--color-copper)" }}>
					<DialogTitle>{t("consent.deleteTitle")}</DialogTitle>
					<DialogDescription>{t("consent.deleteBody")}</DialogDescription>
					<div className="mt-6 flex flex-col gap-2">
						<label htmlFor="consent-delete-confirm" className="luxe-spec-label">
