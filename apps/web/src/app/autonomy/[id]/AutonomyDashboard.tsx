...
import { PhmGroup, type PhmTileProps } from "../../../components/autonomy/PhmTile";
import { SensorSuite } from "../../../components/autonomy/SensorSuite";
import { type SensorReading, SensorTile } from "../../../components/autonomy/SensorTile";
import { TestDriveDebugLog, TestDrivePanel } from "../../../components/autonomy/TestDrivePanel";
...
function DashboardInner({ bookingId }: DashboardProps): React.JSX.Element {
		...
		async function load(): Promise<void> {
			try {
				const res = await fetch(
					`/api/proxy/autonomy/booking/${encodeURIComponent(bookingId)}/grant`,
				);
				if (!res.ok) return;
	...
	const chassis = phm.filter(
		(p) => p.system === "brake" || p.system === "tyres" || p.system === "suspension",
	);
	...
	const overrideCta = (
		<OverrideButton
			...
			{...(grant?.canonicalBytesPreview
				? { canonicalBytesPreview: grant.canonicalBytesPreview }
				: {})}
			onRevoked={(id) => {
				setGrant((g) =>
					g && g.id === id ? { ...g, status: "revoked", ttlRemainingSeconds: 0 } : g,
				);
				toast.push({
			...
			onError={(msg) =>
				toast.push({
					title: t("autonomy.toast.revokeFailed"),
					description: msg,
					tone: "danger",
				})
			}
...
function KpiBand({
	...
	lastTickMs,
	onReconnect,
}: KpiBandProps): React.JSX.Element {
	const t = useTranslations();
	...
	const autonomyState =
		grant?.status === "active" ? "AUTONOMOUS" : grant?.status === "revoked" ? "MANUAL" : "STANDBY";
	const autonomyTone =
		grant?.status === "active" ? "ok" : grant?.status === "revoked" ? "halt" : "watch";
