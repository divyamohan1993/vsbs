...
export function TestDrivePanel({ bookingId }: Props): React.JSX.Element | null {
  ...
  const recording = useMemo(() => {
    ...
    if (!evt) return null;
    const data = evt.data && typeof evt.data === "object" ? (evt.data as Record<string, unknown>) : null;
...
export function TestDriveDebugLog({ bookingId }: Props): React.JSX.Element | null {
  ...
  const { logLines, logConnected, clear } = useBridgeLog(bookingId, { dropNoise: true });
    ...
    <div className="grid gap-3 rounded-[var(--radius-sm)] border border-[var(--color-hairline)] bg-[rgba(0,0,0,0.35)] p-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div>
        ...
        {td.rulP10Seconds != null && td.rulP90Seconds != null ? (
          <p className="luxe-mono text-[length:var(--text-micro)] text-pearl-soft">
            P10 {td.rulP10Seconds.toFixed(0)}s · P50{" "}
            {(td.rulP50Seconds ?? rul ?? 0).toFixed(0)}s · P90{" "}
            {td.rulP90Seconds.toFixed(0)}s
          </p>
        ) : null}
      ...
      <div>
        ...
        <svg viewBox="0 0 240 36" className="mt-1 h-9 w-full" preserveAspectRatio="none">
          {sparkPath ? (
            <path d={sparkPath} stroke="currentColor" strokeWidth="1.5" fill="none" className="text-[var(--color-primary,#c8a560)]" />
          ) : (
            <line x1="0" y1="18" x2="240" y2="18" stroke="currentColor" strokeWidth="1" strokeDasharray="2,3" className="text-pearl-soft opacity-40" />
          )}
        </svg>
        <p className="luxe-mono text-[length:var(--text-micro)] text-pearl-soft">
