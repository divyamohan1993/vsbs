...
export function CommandGrantCard({ grant, override, className }: Props): React.JSX.Element {
	if (!grant) {
		return (
			<GlassPanel
				...
				className={cn("flex flex-col gap-4 !p-8", className)}
			>
				...
				<p className="text-pearl-muted text-[length:var(--text-control)] leading-[1.6]">
					No grant is active for this booking. The vehicle stays under your control until you issue
					one from the booking flow.
				</p>
			</GlassPanel>
		);
	}
	...
	return (
		<GlassPanel
			...
			as="section"
			aria-labelledby="grant-title"
			className={cn("relative grid gap-8 !p-8 md:grid-cols-[1fr_1fr_1fr] md:gap-10", className)}
		>
			<span
				aria-hidden="true"
				className="pointer-events-none absolute inset-x-8 top-0 h-px"
				style={{
					background: "linear-gradient(90deg, transparent, rgba(201,163,106,0.45), transparent)",
				}}
			/>
...
function SealColumn({ grant }: { grant: CommandGrantSummary }): React.JSX.Element {
	...
	return (
		<div className="flex flex-col gap-4">
			...
			<StatusPill tone={STATUS_TONE[grant.status]}>{STATUS_LABEL[grant.status]}</StatusPill>
		</div>
	);
}
...
function ExpiryColumn({
	grant,
	override,
}: { grant: CommandGrantSummary; override?: ReactNode }): React.JSX.Element {
