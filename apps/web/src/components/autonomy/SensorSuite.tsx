...
import type { SensorHealth, TelemetryFrame } from "./useTelemetryStream";
...
function SensorCensus({ frame }: Props): React.JSX.Element {
	...
	return (
		<Section
			title="Sensor suite"
			eyebrow="Surround perception"
			right={<SpecLabel>{countAll(frame)}</SpecLabel>}
		>
...
function SensorGroup({
	label,
	items,
}: { label: string; items: SensorHealth[] }): React.JSX.Element {
	return (
					...
					items.map((s) => (
						<li key={s.id} className="flex items-baseline justify-between gap-3">
							<div className="min-w-0">
								<div className="text-[length:var(--text-control)] text-pearl truncate">
									{s.label}
								</div>
	...
	return (
		<GlassPanel variant="muted" className="relative !p-5">
			<div className="flex items-baseline justify-between">
				...
				<span className="luxe-mono text-[length:var(--text-micro)] uppercase tracking-[var(--tracking-caps)] text-pearl-soft">
					occ {(occ?.occupiedRatio ?? 0).toFixed(2)} · uncert{" "}
					{(occ?.peakUncertainty ?? 0).toFixed(2)}
				</span>
			</div>
		...
		<GlassPanel variant="muted" className="!p-5">
			...
			{lg ? (
				<div className="mt-5 luxe-mono text-[length:var(--text-micro)] uppercase tracking-[var(--tracking-caps)] text-pearl-soft">
					lane {lg.currentLane + 1}/{lg.totalLanes} · conf {lg.confidence.toFixed(2)} · free-space{" "}
					{(fs ?? 0).toFixed(2)}
				</div>
			) : null}
			{planner ? (
				<div className="mt-3 luxe-mono text-[length:var(--text-micro)] uppercase tracking-[var(--tracking-caps)] text-pearl-soft">
					planner · {planner.behavior?.toUpperCase()} · h {planner.horizonS}s ·{" "}
					{planner.sampledTrajectories} alts · cvar95 {(planner.cvar95 ?? 0).toFixed(3)}
				</div>
			) : null}
		</GlassPanel>
	);
...
function DetectionRow({
	...
	return (
		<li className="flex items-baseline justify-between">
			...
			<span
				className="luxe-mono tabular-nums text-pearl"
...
function TracksTable({
	...
	tracks: NonNullable<TelemetryFrame["perception"]>["tracks"];
}): React.JSX.Element {
	if (!tracks) return <></>;
	return (
				...
				<tbody>
					{tracks.map((t) => (
						<tr key={t.id} className="border-t border-[var(--color-hairline)] text-pearl">
							...
							<td className="px-5 py-3 luxe-mono tabular-nums text-[length:var(--text-control)]">
								{t.bearingDeg.toFixed(1)}°
							</td>
							<td className="px-5 py-3 luxe-mono tabular-nums text-[length:var(--text-control)]">
								{t.vMps.toFixed(1)} m/s
							</td>
							<td className="px-5 py-3 luxe-mono tabular-nums text-[length:var(--text-control)]">
								{t.predictionHorizonS ?? "?"} s
							</td>
							<td className="px-5 py-3 luxe-mono tabular-nums text-[length:var(--text-control)]">
								<span
									style={{
										color:
											(t.risk ?? 0) > 0.4 ? "#e6553f" : (t.risk ?? 0) > 0.2 ? "#e8b54e" : "#3fc287",
									}}
								>
									{(t.risk ?? 0).toFixed(2)}
								</span>
				...
				{g ? (
					<GlassPanel variant="muted" className="!p-5">
						...
						<dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
							<Row
								label="Fix"
								value={g.fix.toUpperCase()}
								accent={g.fix.startsWith("rtk") ? "ok" : "warn"}
							/>
							...
							<Row
								label="RTK age"
								value={g.rtkAgeS !== undefined ? `${g.rtkAgeS.toFixed(1)} s` : undefined}
							/>
							<Row
								label="Pos σ"
								value={
									g.posAccuracyM !== undefined
										? `${(g.posAccuracyM * 100).toFixed(1)} cm`
										: undefined
								}
							/>
						</dl>
				...
				{i ? (
					<GlassPanel variant="muted" className="!p-5">
						...
						<dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
							...
							<Row
								label="IMU temp"
								value={i.tempC !== undefined ? `${i.tempC.toFixed(1)} °C` : undefined}
							/>
							<Row
								label="Bias inst."
								value={
									i.biasInstabilityDegHr !== undefined
										? `${i.biasInstabilityDegHr.toFixed(2)} °/h`
										: undefined
								}
							/>
						</dl>
					...
					<GlassPanel variant="muted" className="!p-5">
						...
						<dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
							...
							<Row
								...
								value={
									c.steeringTorqueNm !== undefined
										? `${c.steeringTorqueNm.toFixed(2)} Nm`
										: undefined
								}
							/>
							<Row
								label="Brake p. (front)"
								value={
									c.brakePressureBar?.front !== undefined
										? `${c.brakePressureBar.front.toFixed(0)} bar`
										: undefined
								}
							/>
							<Row
								label="Brake p. (rear)"
								value={
									c.brakePressureBar?.rear !== undefined
										? `${c.brakePressureBar.rear.toFixed(0)} bar`
										: undefined
								}
							/>
							<Row
								label="Friction est."
								value={c.frictionCoef !== undefined ? c.frictionCoef.toFixed(2) : undefined}
							/>
							<Row
								label="Ride h. (avg)"
								value={c.rideHeightMm ? `${avg4(c.rideHeightMm).toFixed(0)} mm` : undefined}
							/>
						</dl>
					</GlassPanel>
				) : null}
...
function CornerTile({
	...
	rpm: number;
	kpa: number;
	hubC: number | undefined;
	tyreC: number | undefined;
}): React.JSX.Element {
	return (
			...
			<div className="grid gap-6 lg:grid-cols-[1fr_2fr]">
				<GlassPanel variant="muted" className="!p-5">
					...
					<dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
						<Row
							label="Front τ"
							value={p.motorFront ? `${p.motorFront.torqueNm.toFixed(0)} Nm` : undefined}
						/>
						<Row
							label="Front stator"
							value={p.motorFront ? `${p.motorFront.tempStatorC.toFixed(1)} °C` : undefined}
						/>
						<Row
							label="Rear τ"
							value={p.motorRear ? `${p.motorRear.torqueNm.toFixed(0)} Nm` : undefined}
						/>
						<Row
							label="Rear stator"
							value={p.motorRear ? `${p.motorRear.tempStatorC.toFixed(1)} °C` : undefined}
						/>
						<Row
							label="Inverter"
							value={p.inverterTempC !== undefined ? `${p.inverterTempC.toFixed(1)} °C` : undefined}
						/>
						<Row
							label="Inverter I"
							value={
								p.inverterCurrentA !== undefined ? `${p.inverterCurrentA.toFixed(0)} A` : undefined
							}
						/>
						<Row
							label="HV bus V"
							value={p.hvBusV !== undefined ? `${p.hvBusV.toFixed(1)} V` : undefined}
						/>
						<Row
							label="HV bus A"
							value={p.hvBusA !== undefined ? `${p.hvBusA.toFixed(0)} A` : undefined}
						/>
						<Row
							label="12V aux"
							value={p.aux12vV !== undefined ? `${p.aux12vV.toFixed(2)} V` : undefined}
						/>
						<Row
							label="Isolation"
							value={p.hvIsolationKohm !== undefined ? `${p.hvIsolationKohm} kΩ` : undefined}
							accent={p.hvIsolationKohm !== undefined && p.hvIsolationKohm < 500 ? "warn" : "ok"}
						/>
						<Row
							label="SoH"
							value={p.hvSohPercent !== undefined ? `${p.hvSohPercent.toFixed(1)} %` : undefined}
						/>
						<Row
							label="SoP"
							value={p.hvSopKw !== undefined ? `${p.hvSopKw.toFixed(0)} kW` : undefined}
						/>
						<Row
							label="Coolant motor"
							value={p.coolantMotorC !== undefined ? `${p.coolantMotorC.toFixed(1)} °C` : undefined}
						/>
						<Row
							label="Coolant batt."
							value={
								p.coolantBatteryC !== undefined ? `${p.coolantBatteryC.toFixed(1)} °C` : undefined
							}
						/>
					</dl>
				</GlassPanel>
...
function CellHeatmap({ mv, tempC }: { mv: number[]; tempC: number[] }): React.JSX.Element {
	...
	return (
		<div className="mt-4">
			...
			<div className="mt-3 luxe-mono text-[length:var(--text-micro)] uppercase tracking-[var(--tracking-caps)] text-pearl-soft">
				min {min} mV · max {max} mV · mean {Math.round(mv.reduce((s, v) => s + v, 0) / mv.length)}{" "}
				mV
			</div>
		</div>
	);
		...
		<Section title="AI compute · lockstep · HSM" eyebrow="On-vehicle silicon">
			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				<GlassPanel variant="muted" className="!p-5">
					...
					<div className="mt-2 text-[length:var(--text-control)] text-pearl">
						{c.primary?.soc ?? "—"}
					</div>
					<Bars
						entries={[
...
function Bars({
	...
	entries: Array<{ label: string; v: number | undefined }>;
}): React.JSX.Element {
	return (
					...
					<div className="mt-1 h-[3px] w-full rounded-full bg-[var(--color-hairline)]">
						<div
							...
							style={{
								...
								background: (e.v ?? 0) > 90 ? "#e6553f" : (e.v ?? 0) > 75 ? "#e8b54e" : "#6ca0d2",
							}}
						/>
					</div>
				</li>
		...
		<Section title="Network · MEC · HD-map" eyebrow="5G NR-V2X · OTA">
			<GlassPanel variant="muted" className="!p-5">
				<dl className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
					...
					<Row
						label="MEC RTT"
						value={n.mecRttMs !== undefined ? `${n.mecRttMs.toFixed(1)} ms` : undefined}
					/>
					<Row
						label="Wi-Fi 6E"
						value={n.wifiRssiDbm !== undefined ? `${n.wifiRssiDbm} dBm` : undefined}
					/>
					<Row
						label="HD-map δ"
						value={
							n.hdMapDeltasPending !== undefined ? `${n.hdMapDeltasPending} pending` : undefined
						}
					/>
				</dl>
				<div className="mt-3 luxe-mono text-[length:var(--text-micro)] uppercase tracking-[var(--tracking-caps)] text-pearl-soft">
		...
		<Section title="V2X bus · PC5 sidelink" eyebrow="C-V2X · 5G NR-V2X">
			<GlassPanel variant="muted" className="!p-5">
				<dl className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
					...
					<Row
						label="Neighbours"
						value={v.neighbours !== undefined ? `${v.neighbours}` : undefined}
					/>
...
function SafetySection({ frame }: Props): React.JSX.Element {
	...
	if (!s) return <></>;
	const oodPct =
		s.oodMahalanobis !== undefined && s.oodThreshold
			? Math.min(1, s.oodMahalanobis / s.oodThreshold)
			: 0;
	return (
			...
			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				...
				<GlassPanel variant="muted" className="!p-5">
					...
					<dl className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
						...
						<Row
							label="TTC"
							value={s.ttcSec !== undefined ? `${s.ttcSec.toFixed(1)} s` : undefined}
						/>
						<Row label="FTTI" value={s.fttiMs !== undefined ? `${s.fttiMs} ms` : undefined} />
						<Row
							label="Capability"
							value={
								s.capabilityBudget !== undefined
									? `${(s.capabilityBudget * 100).toFixed(0)} %`
									: undefined
							}
						/>
			...
			<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
				<GlassPanel variant="muted" className="!p-5">
					...
					{att ? (
						<dl className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
							...
							<Row
								label="Eyes closed"
								value={att.eyesClosed ? "yes" : "no"}
								accent={att.eyesClosed ? "warn" : "ok"}
							/>
							<Row
								label="Hands on wheel"
								value={att.handsOnWheel ? "yes" : "no"}
								accent={att.handsOnWheel === false ? "warn" : "ok"}
							/>
							<Row
								label="Seat belt"
								value={att.seatBelt ? "buckled" : "off"}
								accent={att.seatBelt === false ? "warn" : "ok"}
							/>
						</dl>
					) : (
						<div className="mt-3 text-[length:var(--text-control)] text-pearl-soft">
				...
				<GlassPanel variant="muted" className="!p-5">
					...
					<dl className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
						<Row
							label="Temp"
							value={c.cabinTempC !== undefined ? `${c.cabinTempC.toFixed(1)} °C` : undefined}
						/>
						<Row
							label="RH"
							value={
								c.cabinHumidityPct !== undefined ? `${c.cabinHumidityPct.toFixed(0)} %` : undefined
							}
						/>
						<Row
							label="CO₂"
							value={c.co2Ppm !== undefined ? `${c.co2Ppm} ppm` : undefined}
							accent={c.co2Ppm !== undefined && c.co2Ppm > 1500 ? "warn" : "ok"}
						/>
						<Row
							label="PM2.5"
							value={c.pm25Ugm3 !== undefined ? `${c.pm25Ugm3.toFixed(1)} µg/m³` : undefined}
						/>
					</dl>
				</GlassPanel>
				<GlassPanel variant="muted" className="!p-5">
		...
		<Section title="Environment" eyebrow="Weather · pavement · time-of-day">
			<GlassPanel variant="muted" className="!p-5">
				<dl className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-x-6 gap-y-2 text-[length:var(--text-control)] text-pearl">
					...
					<Row
						label="Visibility"
						value={e.visibilityM !== undefined ? `${e.visibilityM.toLocaleString()} m` : undefined}
					/>
					<Row
						label="Ambient"
						value={e.ambientTempC !== undefined ? `${e.ambientTempC.toFixed(1)} °C` : undefined}
					/>
					<Row
						label="Humidity"
						value={
							e.ambientHumidityPct !== undefined
								? `${e.ambientHumidityPct.toFixed(0)} %`
								: undefined
						}
					/>
					<Row
						label="Wind"
						value={e.windKph !== undefined ? `${e.windKph.toFixed(0)} kph` : undefined}
					/>
