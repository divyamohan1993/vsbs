...
export function RegionSwitcher(): React.JSX.Element {
	...
	if (error) {
		return (
			<p
				role="alert"
				className="rounded-[var(--radius-card)] border border-red-500/40 bg-red-500/10 p-4 text-red-200"
			>
				{error}
			</p>
		);
		...
		<div className="space-y-6">
			<dl className="grid gap-4 md:grid-cols-2">
				<div className="rounded-[var(--radius-card)] border border-muted/30 p-4">
					<dt className="text-muted text-sm uppercase tracking-[0.18em]">{t("region.pinned")}</dt>
					<dd className="mt-1 font-display text-xl">{REGION_LABEL[state.pinned]}</dd>
					<p className="text-muted mt-2 text-sm">{t(`region.reason.${state.reason}`)}</p>
				</div>
				<div className="rounded-[var(--radius-card)] border border-muted/30 p-4">
					<dt className="text-muted text-sm uppercase tracking-[0.18em]">{t("region.detected")}</dt>
