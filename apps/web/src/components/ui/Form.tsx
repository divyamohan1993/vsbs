...
export function Label({ className, ...rest }: LabelHTMLAttributes<HTMLLabelElement>) {
	return (
		<label
			className={cn("luxe-spec-label !text-[length:var(--text-caption)]", className)}
			{...rest}
		/>
	);
}
...
export function Select({ className, children, ...rest }: SelectHTMLAttributes<HTMLSelectElement>) {
	return (
		<select
			className={cn(
				FIELD_BASE,
				"min-h-[56px] appearance-none bg-[var(--color-graphite)]",
				className,
			)}
			{...rest}
		>
...
export function Alert({
	...
	return (
		...
		>
			<p id={titleId} className="font-medium tracking-[var(--tracking-wide)] text-pearl">
				{title}
			</p>
			{children ? (
				<div className="mt-2 text-[length:var(--text-control)] text-pearl-muted leading-[1.6]">
					{children}
				</div>
			) : null}
		</div>
