			...
			{open ? (
				...
				>
					{filtered.length === 0 ? (
						...
						filtered.map((opt, i) => (
							// biome-ignore lint/a11y/useFocusableInteractive: tabIndex={-1} satisfies the WAI-ARIA combobox pattern (focus stays on the input; option focus is virtual via aria-activedescendant)
							// biome-ignore lint/a11y/noNoninteractiveElementToInteractiveRole: standard listbox option pattern
							<li
								key={opt.value}
								id={`${listId}-${i}`}
								role="option"
