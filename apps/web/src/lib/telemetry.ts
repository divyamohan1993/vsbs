...
export function observeVitals(onSample: (s: VitalSample) => void): () => void {
	...
	safeObserve("largest-contentful-paint", (e) => {
		const v =
			(e as PerformanceEntry & { renderTime?: number; startTime: number }).renderTime ??
			e.startTime;
		if (v > lastLcp) {
			lastLcp = v;
			onSample(make("LCP", v));
...
let webTracer: {
	startSpan: (name: string, attrs?: Record<string, unknown>) => ServerSpanLike;
} | null = null;
...
export function recordErrorBoundary(error: Error, info: { componentStack?: string | null }): void {
	webTracer
		?.startSpan("react.error_boundary", {
