...
import { type Rng, type Row, addLaplaceNoise, kAnonymise, seededRng } from "./dp";
...
const QUASI: ReadonlyArray<keyof AnalyticsRow> = ["os", "locale", "cityBand", "route", "theme"];
	...
	return {
		...
		flush(): FlushResult {
			...
			for (const [key, members] of buckets.entries()) {
				const noisyCount = addLaplaceNoise(members.length, 1, epsCount, {
					rng,
				});
				let durSum = 0;
				let depthSum = 0;
				...
				const noisyNDepth = addLaplaceNoise(members.length, 1, epsMeanN, {
					rng,
				});
				out.push({
					key,
