...
export async function loadDraft(draftId: string): Promise<IntakeDraft | undefined> {
	return tx(
		STORE_DRAFTS,
		"readonly",
		async (s) => (await reqAsync(s.get(draftId))) as IntakeDraft | undefined,
	);
}
...
export async function enqueue(
	req: Omit<QueuedRequest, "id" | "attempts" | "enqueuedAt" | "nextAttemptAt">,
): Promise<string> {
	const id = newId();
	const now = Date.now();
...
async function rescheduleWithBackoff(item: QueuedRequest): Promise<void> {
	...
	await tx(STORE_QUEUE, "readwrite", async (s) => {
		await reqAsync(s.put({ ...item, attempts, nextAttemptAt: Date.now() + delayMs }));
	});
}
...
export async function registerServiceWorker(
	...
	navigator.serviceWorker.addEventListener("message", (e) => {
		if (
			...
			e.data &&
			(e.data as { type?: string }).type === "queue-flushed"
		) {
