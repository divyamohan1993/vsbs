...
async function captureFromStream(
	...
	const recorderProc = ctx.createScriptProcessor ? ctx.createScriptProcessor(fftSize, 1, 1) : null;
...
export async function uploadClip(
	...
	const res = await fetch("/api/proxy/intake/audio", {
		method: "POST",
		body: fd,
	});
	if (!res.ok) {
