...
const THERMAL = [{ id: "fir-front", label: "FIR LWIR front", fov: 32, range: 200, hz: 9 }];

const MICS = [{ id: "mic-array", label: "Audio array (8-mic)", hz: 16_000 }];
...
export function buildSyntheticFrame(opts: BuildOpts): LiveTelemetryFrame {
	...
	const wheelJitter = (s: number): number =>
		round1(wheelRpm + Math.sin(t * 1.4 + s) * 1.6 + rng() * 0.6);
	const tpms = (base: number): number =>
		Math.round(base + Math.sin(t / 4 + base) * 1.4 + rng() * 0.4);
	...
	const perception = {
		...
		laneGraph: {
			currentLane: 1,
			totalLanes: 3,
			confidence: round3(0.96 + rng() * 0.02),
		},
		trafficLight:
			phase < 0.15
	...
	return {
		...
		gps: {
			lat: 12.9716 + Math.sin(t / 60) * 0.001,
			lng: 77.5946 + Math.cos(t / 60) * 0.001,
		},
		accel: imu.accel,
		...
		chassis: {
			...
			brakePressureBar: {
				front: round1(control.brake! * 110),
				rear: round1(control.brake! * 70),
			},
