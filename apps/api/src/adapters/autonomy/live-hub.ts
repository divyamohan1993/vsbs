...
const V2xBlock = z
	.object({
		...
		latestKind: z.enum(["BSM", "CAM", "SPaT", "MAP", "DENM", "RSA", "SSM", "TIM"]).optional(),
...
export class LiveAutonomyHub {
	...
	subscribeFrames(bookingId: string, cb: (f: LiveTelemetryFrame) => void): () => void {
		const ch = this.channel(bookingId);
		ch.frameSubscribers.add(cb);
		return () => ch.frameSubscribers.delete(cb);
	}

	subscribeEvents(bookingId: string, cb: (e: PerceptionEvent) => void): () => void {
		const ch = this.channel(bookingId);
