...
import { type GrantChainStoreLike, appendKind, appendRevocation } from "../grant-chain.js";
import type {
	AvpAdapter,
	AvpAuthResult,
	AvpPerformResult,
...
export class MercedesBoschAvpAdapter implements AvpAdapter {
	...
	constructor(cfg: MercedesIppConfig) {
		...
		if (cfg.mode === "live") {
			if (!cfg.base || !cfg.token) {
				throw new Error("Mercedes IPP live mode requires MERCEDES_IPP_BASE and MERCEDES_IPP_TOKEN");
			}
		}
	}
	...
	async revokeGrant(grantId: string, reason: string): Promise<AutonomyAction> {
		...
		if (this.mode === "live") {
			const res = await this.#fetch(`${this.#cfg.base}/v1/grants/${encodeURIComponent(grantId)}`, {
				method: "DELETE",
				headers: { authorization: `Bearer ${this.#cfg.token}` },
			});
