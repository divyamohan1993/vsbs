...
describe("MercedesBoschAvpAdapter sim driver", () => {
	it("walks mint -> accept -> perform -> revoke emitting linked chain entries", async () => {
		...
		const driveResult = await adapter.performScope({
		...
		const diagResult = await adapter.performScope({
			grantId: grant.grantId,
			scope: "diagnose",
		});
	...
	it("readState returns awaiting for unknown vehicle", async () => {
		const adapter = new MercedesBoschAvpAdapter({
			mode: "sim",
			store: new MemoryGrantChainStore(),
		});
