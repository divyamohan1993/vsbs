...
export class RecordingOrchestrator {
	...
	constructor(opts: OrchestratorOptions = {}) {
		...
		this.scriptPath =
			opts.scriptPath ?? path.resolve(this.repoRoot, "tools/carla/scripts/record_demo.sh");
		...
		this.hardTimeoutMsFn = opts.hardTimeoutMs ?? ((durationS) => (durationS + 120) * 1_000);
	}
	...
	private publish(
		id: string,
		partial: Pick<RecordingProgressEvent, "category" | "severity" | "title"> &
			Partial<Pick<RecordingProgressEvent, "detail" | "data">>,
	): void {
	...
	private async finalise(
		...
		const encoder = pickEncoderFromEvents(events) ?? (sizeBytes === 0 ? undefined : "synthetic");
		const wallS = Math.max(0, Math.round((Date.now() - handle.startedAt) / 1_000));

		const errored =
			exitCode !== 0 || sizeBytes === 0 || (last?.severity === "alert" && last.category === "done");
		...
		const finalSummary: RecordingSummary = {
			...
			...(errored
				? {
						errorMessage: last && last.title === "timeout" ? "timeout" : `exit=${exitCode}`,
					}
				: {}),
		};
...
function pickEncoderFromEvents(
	events: RecordingProgressEvent[],
): "hevc_nvenc" | "h264_nvenc" | "libx264" | "libx265" | "synthetic" | undefined {
