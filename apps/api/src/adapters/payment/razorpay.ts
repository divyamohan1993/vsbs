...
import { type OrderStoreLike, buildIntent, buildOrder, transition } from "./state-machine.js";
...
export class RazorpayAdapter implements PaymentAdapter {
	...
	async authorise(
		...
		if (!customerAction.ok) {
			const failed = {
				...
				failureReason: customerAction.reason ?? "declined",
			};
	...
	async refund(orderId: string, reason: string): Promise<PaymentOrder> {
		...
		if (!order) throw new Error(`order ${orderId} not found`);
		const pending = {
			...transition(order, "refund-pending"),
