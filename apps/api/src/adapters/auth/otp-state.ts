...
import type { OtpError, OtpStartRequest, OtpState, OtpVerifyRequest } from "@vsbs/shared";
...
export type OtpVerifyResult = { ok: true; subject: string } | { ok: false; error: OtpError };
...
export function verifyOtp(
	...
	if (s.lockedUntil !== null && now < s.lockedUntil) {
		return {
			...
			error: {
				code: "OTP_LOCKED",
				unlockAt: new Date(s.lockedUntil).toISOString(),
			},
		};
	}
	if (now > s.expiresAt) {
