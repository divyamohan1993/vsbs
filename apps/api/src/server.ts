...
import { REGION_DEFAULT_CONFIG, regionMiddleware } from "./middleware/region.js";
import {
	type AppEnv,
	bodySizeLimit,
	errBody,
...
import { LogBuffer, type LogEntry, buildAdminLogsRouter } from "./routes/admin/logs.js";
...
import { MemoryPendingBookings, buildRegionRouter } from "./routes/region.js";
...
const armorAppEnv: "development" | "production" =
	env.NODE_ENV === "production" ? "production" : "development";
app.use("*", requestId());
app.use("*", cloudArmor({ appEnv: armorAppEnv }));
app.use(
	"*",
	otelMiddleware({
		tracer: otelHandle.tracer,
		region: env.APP_REGION,
		serviceName: "vsbs-api",
	}),
);
...
app.use("*", async (c, next) => {
	...
	const cap =
		path === "/v1/intake/photo" || path === "/v1/intake/audio" ? 5 * 1_048_576 : 1_048_576;
...
app.route(
	...
	buildAdminLogsRouter({
		...
		...(env.GCP_IAP_AUDIENCE !== undefined ? { audience: env.GCP_IAP_AUDIENCE } : {}),
		issuer: env.GCP_IAP_ISSUER,
		jwksUrl: env.GCP_IAP_JWKS_URL,
	}),
);
...
//
// IMPORTANT: gates must register BEFORE the routes they protect (Hono
// matches `app.use` against the prefix in registration order). Mounting
// payments / autonomy / sensors before this block left those routes
// uncovered. Fixed in F3 of the security audit.
...
// Consent for sensor ingest is checked inside routes/sensors.ts /ingest
// after the auth path is resolved. Bearer-authenticated owner ingest
// requires diagnostic-telemetry consent; vehicle-token-authenticated
// producer ingest inherits consent from its booking-bound HMAC. Mounting
// the gate at app level would race the router-internal optionalSession
// and 401 OWNER_REQUIRED the owner-bearer path before the bearer is read.

// -------- payments --------
app.route("/v1/payments", buildPaymentRouter(env, { signingKey: env.SESSION_SIGNING_KEY }));
...
app.route("/v1/bookings", buildBookingsRouter({ signingKey: env.SESSION_SIGNING_KEY }));
...
const recordingsAdminAppEnv: "development" | "test" | "production" =
	env.NODE_ENV === "production" ? "production" : env.NODE_ENV === "test" ? "test" : "development";
app.route(
	"/v1/recordings",
...
app.route("/v1/sensors", buildSensorsRouter(env, { consent: consentManager }));
...
app.post("/v1/intake/commit", zv("json", IntakeSchema), (c) => {
	...
	log.info("intake_committed", {
		id: intake.id,
		ownerHash: hashPhone(intake.owner.phone),
	});
	return c.json({ data: { id: intake.id, status: "accepted" } }, 202);
});

app.post("/v1/intake/draft", zv("json", IntakeDraftSchema), (c) => {
	const draft = c.req.valid("json");
	return c.json({
		data: { draftId: draft.draftId, savedAt: new Date().toISOString() },
	});
});

// The legacy unsigned `POST /v1/autonomy/grant` accept route was removed in
// favour of `POST /v1/autonomy/grant/sign` inside `buildAutonomyRouter`,
// which Zod-validates the full owner-signed CommandGrant, runs the verifier,
// witness-signs, and binds the grant to the authenticated owner subject.
