...
const PHOTO_FIXTURES: Record<z.infer<typeof PhotoKindSchema>, PhotoFinding> = {
	...
	underbody: {
		...
		suggestedActions: ["Replace flange gasket and clamp; inspect mid-pipe for thinning"],
	},
};
...
export function buildIntakeRouter() {
	...
	router.post("/photo", async (c) => {
		...
		if (file.size === 0 || file.size > PHOTO_MAX_BYTES) {
			return c.json(
				errBody("VALIDATION_FAILED", `photo size must be 1..${PHOTO_MAX_BYTES}`, c),
				400,
			);
		}
	...
	router.post("/audio", async (c) => {
		...
		if (clip.size === 0 || clip.size > AUDIO_MAX_BYTES) {
			return c.json(
				errBody("VALIDATION_FAILED", `clip size must be 1..${AUDIO_MAX_BYTES}`, c),
				400,
			);
		}
