...
import { signAdminDevToken } from "../middleware/admin.js";
...
beforeAll(async () => {
	const t = await signAdminDevToken(
		{ subject: "admin-test", roles: ["admin"] },
...
describe("recordings router", () => {
	it("POST /start without admin token rejects 401", async () => {
		const app = buildApp();
