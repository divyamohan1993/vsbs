...
import { TakeoverRungSchema, escalateTakeover } from "@vsbs/shared/takeover";
...
export function buildAutonomyRouter(env: Env, opts: BuildAutonomyRouterOptions = {}) {
	...
	// Owner binding for grants: grantId -> ownerSubject. Set on /grant/sign.
	const grantOwners = new Map<string, string>();

	// Production hard-gate: refuse the sim verifier in prod, even if
	// MERCEDES_IPP_MODE is left as "sim" by mistake. The router still
	// constructs but every grant action throws and the route returns 503.
	const throwOnUseVerifier = () => ({
		mode: "sim" as const,
		verifyOwnerSignature: async (): Promise<boolean> => {
			throw new Error("autonomy grant signer cannot run in sim mode in production");
		},
	});
	const verifier =
	...
	const dualControlResolver: DualControlKeyResolver = opts.dualControlResolver ?? (() => undefined);
	...
	const heartbeatRunner =
		...
		new HeartbeatRunner({
			onRevoke: async (rev: HeartbeatRevocation) => {
				...
				const action = await appendRevocation(
					store,
					rev.grantId,
					`${rev.reason}: ${rev.reasons.join("; ")}`,
				);
				try {
	...
	// Live telemetry hub — used by ingest + SSE routes below.
	...
	/** Owner ↔ grant binding check. Returns null when allowed, Response otherwise. */
	const ensureGrantOwner = (
		...
		if (!ownerSubject) {
			return new Response(
				JSON.stringify(errBody("SESSION_REQUIRED", "Authorization required", c)),
				{ status: 401, headers: { "content-type": "application/json" } },
			);
		}
		const owner = grantOwners.get(grantId);
		if (owner !== undefined && owner !== ownerSubject) {
			return new Response(JSON.stringify(errBody("GRANT_FORBIDDEN", "Not your grant", c)), {
				status: 403,
	...
	/**
	 * Verify a vehicle producer token. Recomputes
	 * HMAC-SHA-256(SESSION_SIGNING_KEY, `${bookingId}.${sha256(body)}`) and
	 * constant-time compares against the supplied b64url header. The chaos
	 * driver and CARLA bridge mint this header with the same key.
	 */
	const verifyVehicleProducerToken = async (
		...
		const expected = await hmacSha256(env.SESSION_SIGNING_KEY, `${bookingId}.${bodyHashB64u}`);
		return constantTimeStringEquals(expected, headerToken);
	};

	// ----- Session gate for owner-driven grant routes --------------------------
	...
	router.post(
		...
		async (c) => {
			...
			const ok = await verifyVehicleProducerToken(bookingId, raw, headerToken);
			if (!ok) {
				return c.json(errBody("VEHICLE_TOKEN_INVALID", "Vehicle producer token invalid", c), 401);
			}
			let parsed: LiveTelemetryFrame;
			try {
				const json = JSON.parse(raw);
				parsed = LiveTelemetryFrameSchema.parse(json);
			} catch (err) {
				return c.json(errBody("VALIDATION_FAILED", "Telemetry frame invalid", c, String(err)), 400);
			}
			hub.publishFrame(bookingId, parsed);
			return c.json({ data: { ok: true, ts: parsed.ts } }, 202);
		},
	);
	...
	router.post(
		...
		async (c) => {
			...
			try {
				const json = JSON.parse(raw);
				parsed = PerceptionEventSchema.parse(json);
			} catch (err) {
				return c.json(
					errBody("VALIDATION_FAILED", "Perception event invalid", c, String(err)),
					400,
				);
			}
			hub.publishEvent(bookingId, parsed);
			return c.json({ data: { ok: true, ts: parsed.ts } }, 202);
		},
	);

	// ---------- Capability v2 (read-only, owner session) ---------------------
	// Mounted under sessionGate via router.use below.

	// ---------- Demo / dashboard read paths ----------------------------------
	// The grant-by-booking route renders a placeholder when no live OEM is
	// connected; it is intentionally NOT guarded by AUTONOMY_ENABLED but it
	// does require a session and that the booking belong to the caller.
	...
	router.get(
		...
		(c) => {
			...
			if (owner !== null && owner !== ownerSubject) {
	...
	// All remaining routes require an authenticated owner session.
	router.use("*", sessionGate);
	...
	router.post("/grant/sign", zv("json", CommandGrantSchema), async (c) => {
		...
		let ok: boolean;
		try {
			ok = await verifier.verifyOwnerSignature(grant, null);
		} catch (err) {
			return c.json(errBody("AUTONOMY_NOT_AVAILABLE", String(err), c), 503);
		}
		if (!ok) {
		...
		grantOwners.set(mergedGrant.grantId, c.get("ownerSubject"));
	...
	router.post(
		...
		async (c) => {
			...
			const ownerBlocked = ensureGrantOwner(
	...
	router.post(
		...
		async (c) => {
			...
			if (!heartbeatRunner.isRunning(id)) {
				...
				const cell: { tier1Healthy: boolean; reasons: string[] } = {
					tier1Healthy: true,
					reasons: [],
				};
				heartbeatRunner.start(id, { intervalMs: 1_000 }, async () => ({
					tier1Healthy: cell.tier1Healthy,
	...
	router.post(
		...
		async (c) => {
			...
			if (grant.grantId !== id) {
				return c.json(
					errBody("GRANT_ID_MISMATCH", "URL grantId does not match body grantId", c),
					400,
				);
			}
...
/** Constant-time string compare. Both inputs must be ASCII-safe. */
