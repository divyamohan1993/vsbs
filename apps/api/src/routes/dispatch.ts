...
const ShortlistBodySchema = z.object({
	...
	mode: z.enum(["drive-in", "mobile", "tow", "autonomous-tier-a"]).default("drive-in"),
...
export function buildDispatchRouter(deps: DispatchRouterDeps = {}) {
	...
	router.post("/shortlist", zv("json", ShortlistBodySchema), (c) => {
		...
		if (triaged.length === 0) {
			return c.json(
				errBody("NO_SC_HAS_PARTS", "No service centre has every required part in stock.", c, {
					requiredParts,
				}),
				409,
			);
	...
	router.get("/:bookingId", zv("param", z.object({ bookingId: z.string().uuid() })), (c) => {
		const { bookingId } = c.req.valid("param");
