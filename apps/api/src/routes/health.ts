...
export function buildHealthRouter(deps: HealthRouterDeps) {
	...
	app.get("/readyz", async (c) => {
		...
		const status = report.status === "unhealthy" ? 503 : report.status === "degraded" ? 200 : 200;
		return c.json(
			{
	...
	detailApp.use("*", adminOnly({ mode: deps.adminAuthMode, appEnv: deps.appEnv }));
	...
	app.notFound((c) => c.json(errBody("HEALTH_NOT_FOUND", "Health route not found", c), 404));
