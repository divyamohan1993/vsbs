...
import { type AdminAppEnv, adminOnly } from "../middleware/admin.js";
import { type AppEnv, errBody } from "../middleware/security.js";
...
export interface BuildRecordingsRouterOptions {
	...
	/**
	 * Admin gate options. The router applies `adminOnly(...)` to every route —
	 * starting / listing / fetching / streaming a demo recording is an
	 * operator action, not an end-user one. In sim mode the dev-token short
	 * path applies; in live mode IAP is required.
	 */
	admin: {
		mode: "sim" | "live";
...
export function buildRecordingsRouter(opts: BuildRecordingsRouterOptions) {
	...
	const router = new Hono<AppEnv & AdminAppEnv>();

	// Every route is admin-gated.
	router.use(
		"*",
		adminOnly({
			mode: opts.admin.mode,
			appEnv: opts.admin.appEnv,
			...(opts.admin.signingKey !== undefined ? { signingKey: opts.admin.signingKey } : {}),
			...(opts.admin.audience !== undefined ? { audience: opts.admin.audience } : {}),
			...(opts.admin.issuer !== undefined ? { issuer: opts.admin.issuer } : {}),
			...(opts.admin.jwksUrl !== undefined ? { jwksUrl: opts.admin.jwksUrl } : {}),
		}),
	);
	...
	router.get("/:id/progress/sse", zv("param", IdParam), (c) => {
		...
		return streamSSE(c, async (stream) => {
			...
			if (indexSummary && (indexSummary.status === "done" || indexSummary.status === "error")) {
				await stream.writeSSE({
		...
		const stream =
			typeof (
				globalThis as unknown as {
					Bun?: { file: (p: string) => { stream: () => ReadableStream } };
				}
			).Bun?.file === "function"
				? (
						globalThis as unknown as {
