...
import { MemoryDispatchLegStore, buildDispatchRouter } from "./dispatch.js";
...
describe("dispatch routes", () => {
	...
	it("leg state machine advances en-route -> at-sc -> servicing -> serviced -> returning -> closed", async () => {
		...
		const arrive = await app.request(`/v1/dispatch/${BOOKING_ID}/arrive`, {
			method: "POST",
		});
		...
		const complete = await app.request(`/v1/dispatch/${BOOKING_ID}/complete`, {
			method: "POST",
		});
		expect((await complete.json()).data.leg).toBe("serviced");

		const ret = await app.request(`/v1/dispatch/${BOOKING_ID}/return-leg`, {
			method: "POST",
		});
		expect((await ret.json()).data.leg).toBe("returning");

		const closed = await app.request(`/v1/dispatch/${BOOKING_ID}/returned`, {
			method: "POST",
		});
	...
	it("rejects illegal leg transitions with 409", async () => {
		...
		const skip = await app.request(`/v1/dispatch/${BOOKING_ID}/complete`, {
			method: "POST",
		});
