...
import { type ComponentId, type PhmReading, PhmReadingSchema, phmAction } from "@vsbs/shared";
...
const TRIGGER_BY_COMPONENT: Partial<Record<ComponentId, PhmTriggerSpec>> = {
	"brakes-pads-front": {
		symptom:
			"Front brake-pad wear is past minimum thickness — pedal feel and stopping distance compromised.",
...
export function draftBookingFromPhm(reading: PhmReading): {
	...
	draft: {
		...
		issue: {
			symptoms: string;
			canDriveSafely: "yes-cautiously" | "no" | "unsure";
			redFlags: string[];
		};
		safety: {
			severity: "red" | "amber" | "green";
	...
	if (
		action.kind === "takeover-required-and-block-autonomy" ||
		action.kind === "manual-drive-to-shop"
	) {
		severity = "red";
		canDrive = "no";
...
export function buildPhmRouter() {
	...
	router.post("/actions", zv("json", PhmEvalSchema), (c) => {
		...
		const actions = readings.map((r) => ({
			component: r.component,
			action: phmAction(r, inMotion),
		}));
	...
	router.post(
		...
		(c) => {
			...
			if (body.vehicleId !== vehicleId) {
				return c.json(errBody("VEHICLE_ID_MISMATCH", "Path and body vehicleId disagree", c), 400);
			}
			try {
				...
				return c.json(
					{
						data: {
							spec,
							draft,
							action: phmAction(body.reading, body.inMotion),
						},
					},
					201,
				);
