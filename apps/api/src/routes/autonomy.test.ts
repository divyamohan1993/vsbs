...
import { type CommandGrant, CommandGrantSchema } from "@vsbs/shared";
import {
...
import { canonicalGrantBytes, simSignOwner } from "@vsbs/shared/commandgrant-lifecycle";
...
async function mintGrant(): Promise<CommandGrant> {
	...
	return CommandGrantSchema.parse({
		...baseTemplate,
...
async function buildAppWithGrant(
	witnessKeys?: {
	...
	const router = buildAutonomyRouter(baseEnv, {
		...(witnessKeys
			? {
					witnessKeys: witnessKeys.signing,
					witnessVerifyingKeys: witnessKeys.verifying,
				}
			: {}),
...
describe("autonomy heartbeat route", () => {
	it("rejects unauthenticated heartbeat with 401", async () => {
	...
	it("healthy beat returns ok+not-revoked", async () => {
		const { app, grant, auth } = await buildAppWithGrant();
		const res = await app.request(`/v1/autonomy/grants/${grant.grantId}/heartbeat`, {
			method: "POST",
			headers: { "content-type": "application/json", authorization: auth },
			body: JSON.stringify({ tier1Healthy: true, reasons: [] }),
	...
	it("404 for unknown grant id", async () => {
		const { app, auth } = await buildAppWithGrant();
	...
	it("rejects another owner with 403 GRANT_FORBIDDEN", async () => {
		const { app, grant } = await buildAppWithGrant();
		const otherAuth = await bearer("attacker-2");
		const res = await app.request(`/v1/autonomy/grants/${grant.grantId}/heartbeat`, {
			method: "POST",
			headers: {
		...
		const body = await res.json();
		expect(body.error.code).toBe("GRANT_FORBIDDEN");
	});
});
...
describe("autonomy dual-control route", () => {
	it("first signature returns pending; quorum mints grant", async () => {
		...
		const { app, grant, auth } = await buildAppWithGrant(undefined, resolver);
