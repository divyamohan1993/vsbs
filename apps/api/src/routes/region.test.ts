...
import { MemoryPendingBookings, buildRegionRouter } from "./region.js";
...
function buildApp(opts?: {
	pending?: MemoryPendingBookings;
	runtime?: "asia-south1" | "us-central1";
}) {
	...
	app.route("/v1/region", buildRegionRouter({ signingKey: SIGN_KEY, router, pending }));
	return { app, pending };
}
	...
	it("returns detected + pinned + allowedSwitch=true when no pending bookings", async () => {
		...
		const res = await app.request("/v1/region/me", {
			headers: {
				...
				authorization: await bearer("u-region-1"),
			},
		});
...
describe("POST /v1/region/switch", () => {
	it("switches the pinned region, sets the cookie, returns the new base URL", async () => {
		...
		const res = await app.request("/v1/region/switch", {
			...
			headers: {
				...
				authorization: await bearer("u-region-2"),
			},
	...
	it("rejects an invalid target region with 400", async () => {
		...
		const res = await app.request("/v1/region/switch", {
			...
			headers: {
				"content-type": "application/json",
				authorization: await bearer("u-region-3"),
			},
	...
	it("treats a same-region switch as a no-op but still refreshes the cookie", async () => {
		...
		const res = await app.request("/v1/region/switch", {
			...
			headers: {
				...
				authorization: await bearer("u-region-4"),
			},
	...
	it("ignores any client-supplied ownerId — identity comes from the session", async () => {
		const pending = new MemoryPendingBookings();
		pending.setPending("alice", 1);
		const { app } = buildApp({ pending });
		// Forge a body ownerId of "bob" (no pending) but authenticate as alice (1 pending).
		const res = await app.request("/v1/region/switch", {
			method: "POST",
			headers: {
				"content-type": "application/json",
			...
			body: JSON.stringify({ to: "asia-south1", ownerId: "bob" }),
		});
		// Must still 409 because alice (the session subject) is the only identity that counts.
		expect(res.status).toBe(409);
		const body = await res.json();
		expect(body.error.code).toBe("REGION_SWITCH_BLOCKED");
	});
