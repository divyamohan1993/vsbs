...
export interface AdminRouterDeps {
	...
	audience?: string;
	/** Expected IAP issuer. Defaults inside `adminOnly`. */
	issuer?: string;
	/** JWKs URL. Defaults inside `adminOnly`. */
	jwksUrl?: string;
}
...
export function buildAdminRouter(deps: AdminRouterDeps) {
	...
	router.use(
		"*",
		adminOnly({
			mode: deps.adminAuthMode,
			appEnv: deps.appEnv,
			...(deps.signingKey !== undefined ? { signingKey: deps.signingKey } : {}),
			...(deps.audience !== undefined ? { audience: deps.audience } : {}),
			...(deps.issuer !== undefined ? { issuer: deps.issuer } : {}),
			...(deps.jwksUrl !== undefined ? { jwksUrl: deps.jwksUrl } : {}),
		}),
	);
	...
	router.get("/bookings", zv("query", BookingListQuery), (c) => {
		...
		return c.json({
			data: page,
			page: { total: filtered.length, nextCursor, limit: q.limit },
		});
	});
	...
	// ---- Capacity heat map ----
	router.get("/capacity/heatmap", zv("query", z.object({ scId: z.string().optional() })), (c) => {
	...
	// ---- Slots ----
	router.get("/slots", zv("query", z.object({ scId: z.string().optional() })), (c) => {
		const { scId } = c.req.valid("query");
	...
	router.delete("/slots/:id", zv("param", z.object({ id: z.string().min(1) })), (c) => {
		const { id } = c.req.valid("param");
	...
	// ---- Pricing ----
	router.get("/pricing/:scId", zv("param", z.object({ scId: z.string().min(1) })), (c) => {
	...
	router.post("/pricing/transition", zv("json", PricingTransitionSchema), (c) => {
		...
		if (body.to === "published" && version.state !== "review") {
			return c.json(
				errBody("PRICING_BAD_TRANSITION", "Only reviewed versions can publish", c),
				409,
			);
		}
	...
	router.notFound((c) => c.json(errBody("ADMIN_ROUTE_NOT_FOUND", "Admin route not found", c), 404));
...
function computeMerkleProof(
	...
	siblings: Array<{ hex: string; side: "left" | "right" }>;
	rootHex: string;
} {
