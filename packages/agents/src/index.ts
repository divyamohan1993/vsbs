...
import { ToolRegistry, type VsbsHttpClient, createHttpClient } from "./tools/registry.js";
...
export interface VsbsGraphHandle {
	...
	runTurn(state: { messages: LlmMessage[] }, input: ConciergeTurnInput): AsyncIterable<AgentEvent>;
...
export function buildVsbsGraph(opts: BuildVsbsGraphOptions): VsbsGraphHandle {
	const http: VsbsHttpClient =
		opts.http ?? createHttpClient(opts.apiBase, opts.defaultHeaders ?? {});
