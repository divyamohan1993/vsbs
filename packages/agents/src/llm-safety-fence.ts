...
export function extractSensorFlagsFromToolResults(results: ToolResult[]): {
	...
	return {
		redFlags,
		amberFlags,
		phmRaisedUnsafeOrCritical,
		tierOneSensorFailure,
	};
}
...
export interface SafetyFenceVerdict {
	...
	reasons: Array<
		"deterministic-red-flag" | "phm-unsafe" | "tier-1-sensor-failure" | "internal-error"
	>;
		...
		try {
			...
			if ((phmBlocked || tierOneDead) && looksLikeDriveSuggestion(candidate.content)) {
				return {
					message: {
						role: "assistant",
						content: CANONICAL_DO_NOT_DRIVE_ADVISORY,
					},
					verdict: {
						overridden: true,
						reasons,
