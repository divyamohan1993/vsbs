...
import { z } from "zod";
import { type ToolResultEnvelope, envelope } from "../confidence.js";
import type { ToolRegistry, VsbsHttpClient } from "./registry.js";
...
export function registerVsbsTools(registry: ToolRegistry): void {
	...
	registry.register({
		...
		handler: async (args, http) => {
			...
			const res = await http.post(
				`/v1/payments/orders/${encodeURIComponent(orderId)}/intents`,
				body,
			);
			const value = await readJson(res);
			return envelope(value, {
	...
	registry.register({
		...
		handler: async (args, http) => {
			...
			const res = await http.post(
				`/v1/payments/intents/${encodeURIComponent(intentId)}/authorise`,
				body,
			);
			const value = await readJson(res);
			return envelope(value, {
	...
	registry.register({
		...
		handler: async (args, http) => {
			const res = await http.post(
				`/v1/payments/orders/${encodeURIComponent(args.orderId)}/capture`,
				{},
			);
			const value = await readJson(res);
			return envelope(value, {
