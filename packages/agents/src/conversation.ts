...
import type { AgentEvent, ConciergeTurnInput, ToolResult } from "./types.js";
...
export async function* runOneTurn(
	...
	const finalize = (candidate: LlmMessage): LlmMessage => {
		try {
			...
			if (gate.belowFloor) {
				working = {
					role: "assistant",
					content: CANONICAL_LOW_CONFIDENCE_ADVISORY,
				};
			}
	...
	const messages: LlmMessage[] = [...state.messages, { role: "user", content: input.userMessage }];
