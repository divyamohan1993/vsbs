...
export function recombineShares(shares: Share[], threshold: number): Uint8Array {
	...
	if (!first) throw new Error("no shares decoded");
	const secretLen = first.length - 2;
 ...
 */
export function verifyCeremonyRecord(record: CeremonyRecord): {
	ok: boolean;
	reason?: string;
} {
