...
import { type CommandGrant, canonicalGrantBytes } from "@vsbs/shared";
import { ML_DSA_65_ALG, type PqSigner, makeMlDsa65Signer } from "./sig.js";
import {
...
export async function verifyPasskeyGrantAssertion(
	...
	if (!eqBytes(challengeFromInput, expectedChallengeBytes)) {
		return {
			ok: false,
			reason: "challenge does not match canonical grant bytes",
		};
	}
