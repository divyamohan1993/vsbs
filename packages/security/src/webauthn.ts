...
export function cborDecode(buf: Uint8Array, offset = 0): CborDecodeResult {
	...
	const readUint = (m: number): { v: number; off: number } => {
		...
		if (m === 26) {
			...
			const v =
				buf[off]! * 0x1000000 + ((buf[off + 1]! << 16) | (buf[off + 2]! << 8) | buf[off + 3]!);
			return { v, off: off + 4 };
		}
		if (m === 27) {
	...
	switch (major) {
		...
		case 3: {
			...
			return {
				value: new TextDecoder("utf-8").decode(buf.subarray(o, o + len)),
				next: o + len,
			};
		}
		case 4: {
...
export function cborEncode(value: unknown): Uint8Array {
	...
	function writeHead(major: number, n: number): void {
		if (n < 24) parts.push((major << 5) | n);
			...
			parts.push((major << 5) | 24);
			parts.push(n & 0xff);
		} else if (n < 0x10000) {
			parts.push((major << 5) | 25);
			parts.push((n >> 8) & 0xff, n & 0xff);
...
export async function makeRegistrationFixture(input: {
	...
	const authData = new Uint8Array(37 + 16 + 2 + credentialId.length + coseBytes.length);
