...
export class MemoryStore implements RateLimitStore {
	...
	async hit(
		...
		windowMs: number,
		now: number,
	): Promise<{ effective: number; resetMs: number }> {
...
function pickRoute(
	path: string,
	perRoute: Record<string, RouteRateLimit> | undefined,
	fallback: RouteRateLimit,
): RouteRateLimit {
...
function buildKey(
	...
	switch (by) {
		...
		case "user":
			return `rl:user:${user ?? "anon"}:${route}`;
		case "ip+user":
			return `rl:ipuser:${ip}|${user ?? "anon"}:${route}`;
	}
}
		...
		middleware(): MiddlewareHandler {
			...
			return async (c: Context, next: () => Promise<void>) => {
				...
				c.header(
					...
					String(Math.max(0, Math.floor(decision.max - decision.effective))),
				);
