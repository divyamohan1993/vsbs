...
import { z } from "zod";
import { type CommandGrant, CommandGrantSchema } from "./autonomy.js";
...
export const OfflineGrantEnvelopeSchema = z
	.object({
		...
		offlineTtlMs: z.number().int().positive().max(OFFLINE_GRANT_MAX_TTL_MS),
...
export function mintOfflineEnvelope(input: MintOfflineEnvelopeInput): OfflineGrantEnvelope {
	const ttl = Math.min(input.offlineTtlMs ?? OFFLINE_GRANT_MAX_TTL_MS, OFFLINE_GRANT_MAX_TTL_MS);
...
export function permitOfflineAction(input: PermitOfflineActionInput): PermitOfflineActionResult {
	...
	if (!allowedSet.has(input.action)) {
		return {
			permitted: false,
			reason: `action ${input.action} not in envelope allowedActions`,
		};
	}
