...
import { SAFETY_RED_FLAGS, assessSafety, postCheckSafetyAgrees } from "./safety.js";
...
describe("assessSafety", () => {
	it("returns green when no signals reported", () => {
		const r = assessSafety({
			owner: { canDriveSafely: "yes-confidently", redFlags: [] },
		});
	...
	it("sensor brake-pressure-residual-critical forces red", () => {
		const r = assessSafety({
			sensorFlags: ["brake-pressure-residual-critical"],
		});
		expect(r.severity).toBe("red");
	...
	it("unknown owner flag is ignored", () => {
		const r = assessSafety({
			owner: { canDriveSafely: "yes-confidently", redFlags: ["nonsense"] },
		});
		expect(r.severity).toBe("green");
	});
...
describe("assessSafety — historical regression suite", () => {
	it.each(REG_FIXTURE.cases.map((c) => [c.id, c]))("%s", (_id, kase) => {
		...
		if (kase.expect.rationaleMatches) {
			expect(result.rationale.toLowerCase()).toContain(kase.expect.rationaleMatches.toLowerCase());
		}
	});
...
describe("postCheckSafetyAgrees", () => {
	it("agrees with identical inputs (green)", () => {
		const raw = {
			owner: { canDriveSafely: "yes-confidently" as const, redFlags: [] },
		};
	...
	it("rejects when triggered count mismatches", () => {
		...
		expect(
			postCheckSafetyAgrees(primary, {
				owner: { redFlags: ["brake-failure", "engine-fire"] },
			}),
		).toBe(false);
	});
});
