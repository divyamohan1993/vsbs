...
export function resolveAutonomyCapabilityV2(
	...
	if (!entry) {
		return {
			tier: base.tier,
			eligible: false,
			reason: `OEM "${ctx.oemId}" not present in capability registry.`,
		};
	}
	...
	if (!entry.vehicleYearsSupported.includes(ctx.vehicle.year)) {
		return {
			tier: base.tier,
			eligible: false,
			reason: "Vehicle year is outside the OEM's cleared range.",
		};
	}
	const allHw = entry.autonomyHwRequired.every(
		(hw) => ctx.vehicle.autonomyHw?.includes(hw) ?? false,
	);
	if (!allHw) {
		return {
			tier: base.tier,
			eligible: false,
			reason: "Vehicle lacks required autonomy hardware per OEM registry.",
		};
	}
	...
	if (entry.insuranceGate && !ctx.owner.insuranceAllowsAutonomy) {
		return {
			tier: base.tier,
			eligible: false,
			reason: "OEM requires insurance clearance; owner policy does not cover autonomy.",
		};
	}
	...
	if (!geo) {
		return {
			tier: base.tier,
			eligible: false,
			reason: "Destination provider has no geofence in the catalogue.",
		};
	}
	...
	return {
		tier: base.tier,
		eligible: true,
		reason: "All v2 gates cleared — OEM, hardware, provider, insurance, geofence.",
	};
}
