...
import { SEED_PASSENGER_LIGHT_MANIFEST, SEED_TWO_WHEELER_MANIFEST } from "./coverage-manifest.js";
import { type PhmReading, isTierOneSensorDead, phmAction, phmActionWithCoverage } from "./phm.js";
...
describe("phmAction", () => {
	it("healthy → silent", () => {
		expect(phmAction(reading({ state: "healthy" }), false)).toEqual({
			kind: "silent",
		});
	});

	it("watch → remind-next-open", () => {
		expect(phmAction(reading({ state: "watch" }), false)).toEqual({
