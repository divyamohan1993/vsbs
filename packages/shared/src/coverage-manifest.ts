...
import { z } from "zod";
import { type VehicleClass, VehicleClassSchema } from "./odd.js";
import { COMPONENT_TIER, type ComponentId, ComponentIdSchema, type Tier } from "./phm.js";
...
const TIER_3_PASSENGER_LIGHT: ComponentId[] = ["suspension-dampers", "exhaust-o2", "dpf"];
...
export const SEED_TWO_WHEELER_MANIFEST: CoverageManifest = {
	...
	knownGaps: [
		{
			component: "abs-module",
			reason: "Single-channel ABS only on select trims; calibration per VIN-derived trim pending.",
		},
		{
			component: "airbag-srs",
...
export class CoverageGap extends Error {
	...
	constructor(vehicleClass: VehicleClass, missing: ComponentId[]) {
		super(`Tier-1 components uncovered for vehicle class ${vehicleClass}: ${missing.join(", ")}`);
