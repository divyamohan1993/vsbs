...
import { z } from "zod";
import { AUTONOMY_MAX_GEOFENCE_METERS, AUTONOMY_MAX_GRANT_SECONDS } from "./constants.js";
...
export const GeofenceSchema = z.object({
	...
	radiusMeters: z.number().positive().max(AUTONOMY_MAX_GEOFENCE_METERS),
});
...
export function resolveAutonomyCapability(ctx: AutonomyCapabilityContext): AutonomyCapability {
	if (!ctx.owner.autonomyConsentGranted) {
		return {
			tier: "A-AVP",
			eligible: false,
			reason: "Owner has not consented to autonomy delegation.",
		};
	}
	if (!ctx.owner.insuranceAllowsAutonomy) {
		return {
			tier: "A-AVP",
			eligible: false,
			reason: "Owner's insurance policy does not cover autonomous operation.",
		};
	}
	if (!ctx.providersSupported.includes(ctx.destinationProvider)) {
		return {
			tier: "A-AVP",
			eligible: false,
			reason: "Destination service center does not host an approved AVP provider.",
		};
	}
	if (!ctx.vehicle.yearsSupported.includes(ctx.vehicle.year)) {
		return {
			tier: "A-AVP",
			eligible: false,
			reason: "Vehicle year/trim is not in the supported list for this provider.",
		};
	}
	...
	if (!has("intelligent-park-pilot")) {
		return {
			tier: "A-AVP",
			eligible: false,
			reason: "Vehicle is not equipped with Intelligent Park Pilot.",
		};
	}
	return {
		tier: "A-AVP",
		eligible: true,
		reason: "All gates cleared — Tier A AVP available.",
	};
}
