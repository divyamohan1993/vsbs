...
describe("resolveAutonomyCapabilityV2", () => {
		...
		const r = resolveAutonomyCapabilityV2(
			{
				...
				vehicle: {
					...baseCtx.vehicle,
					year: 2022,
					yearsSupported: [2022, 2023, 2024, 2025, 2026],
				},
			},
			SEED_OEM_REGISTRY,
			SEED_GEOFENCE_CATALOGUE,
	...
	it("provider not in OEM approved list fails", () => {
		const r = resolveAutonomyCapabilityV2(
			{
				...baseCtx,
				destinationProvider: "other-provider",
				providersSupported: ["other-provider"],
			},
			SEED_OEM_REGISTRY,
			SEED_GEOFENCE_CATALOGUE,
		);
	...
	it("insurance gate off when OEM requires it fails", () => {
		const r = resolveAutonomyCapabilityV2(
			{
				...baseCtx,
				owner: { autonomyConsentGranted: true, insuranceAllowsAutonomy: false },
			},
			SEED_OEM_REGISTRY,
			SEED_GEOFENCE_CATALOGUE,
		);
...
describe("signed geofence entries (E3)", () => {
	...
	it("loadVerifiedCatalogue surfaces reasons for rejected entries", () => {
		...
		const result = loadVerifiedCatalogue(
			[goodSigned, badSigned],
			resolver,
