...
export interface GrantSignatureVerifier {
	...
	verifyOwnerSignature(grant: CommandGrant, publicKey: JsonWebKey | null): Promise<boolean>;
}
	...
	return {
		...
		async verifyOwnerSignature(
			...
			switch (grant.ownerSigAlg) {
				...
				case "ed25519": {
					const key = await crypto.subtle.importKey("jwk", publicKey, { name: "Ed25519" }, false, [
						"verify",
					]);
					return crypto.subtle.verify(
						{ name: "Ed25519" },
...
async function chainHashOf(prev: string | undefined, payloadHash: string): Promise<string> {
	...
	for (let i = 0; i < prevHex.length / 2; i++) {
		both[i] = Number.parseInt(prevHex.slice(i * 2, i * 2 + 2), 16);
	}
	const offset = prevHex.length / 2;
	for (let i = 0; i < payloadHash.length / 2; i++) {
		both[offset + i] = Number.parseInt(payloadHash.slice(i * 2, i * 2 + 2), 16);
	}
