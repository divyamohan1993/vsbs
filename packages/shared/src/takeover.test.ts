...
describe("buildTakeoverPrompt", () => {
	...
	it("adds modalities monotonically across the ladder", () => {
		...
		expect(info.modalities).toEqual({
			visual: true,
			auditory: false,
			tactile: false,
			haptic: false,
		});
		expect(warn.modalities).toEqual({
			visual: true,
			auditory: true,
			tactile: false,
			haptic: false,
		});
		expect(urg.modalities).toEqual({
			visual: true,
			auditory: true,
			tactile: true,
			haptic: true,
		});
		expect(mrm.modalities).toEqual({
			visual: true,
			auditory: true,
			tactile: true,
			haptic: true,
...
describe("escalateTakeover", () => {
	it("holds when elapsed is within the rung window", () => {
		expect(escalateTakeover("informational", 500, false)).toEqual({
			kind: "hold",
			rung: "informational",
		});
	});
	...
	it("ack short-circuits escalation", () => {
		expect(escalateTakeover("urgent", 999_999, true)).toEqual({
			kind: "acknowledged",
			rung: "urgent",
		});
	});
	...
	it("escalates informational -> warning", () => {
		...
		expect(escalateTakeover("informational", p.maxHoldMs, false)).toEqual({
			kind: "escalate",
			rung: "warning",
		});
	});
	...
	it("escalates warning -> urgent", () => {
		...
		expect(escalateTakeover("warning", p.maxHoldMs, false)).toEqual({
			kind: "escalate",
			rung: "urgent",
		});
	});
	...
	it("urgent rung elapsed triggers MRM", () => {
		...
		expect(escalateTakeover("urgent", p.maxHoldMs, false)).toEqual({
			kind: "mrm-triggered",
		});
	});

	it("emergency-mrm is terminal", () => {
		expect(escalateTakeover("emergency-mrm", 0, false)).toEqual({
			kind: "mrm-triggered",
		});
		expect(escalateTakeover("emergency-mrm", 0, true)).toEqual({
			kind: "mrm-triggered",
		});
	});
});
...
describe("minimumRiskManeuver", () => {
	it("hands to driver when driver present and in ODD", () => {
		const plan = minimumRiskManeuver({
			speedMps: 15,
			inOdd: true,
			hardShoulderReachable: true,
			driverPresent: true,
		});
