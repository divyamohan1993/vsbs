...
export const OtpVerifyResponseSchema = z.object({
	...
	purpose: OtpPurposeSchema,
	/**
	 * HMAC-signed bearer token minted on successful verification.
	 * Optional for backward compatibility with older callers; the api/auth.ts
	 * route always populates it. Clients send it back as
	 * `Authorization: Bearer <sessionToken>` to owner-scoped endpoints.
	 */
	sessionToken: z.string().optional(),
	/** ISO-8601 expiry of `sessionToken`. Always present when sessionToken is. */
