...
describe("OperationalDesignDomainSchema", () => {
	...
	it("rejects empty axis allow-lists", () => {
		expect(() => OperationalDesignDomainSchema.parse({ ...baseOdd, region: [] })).toThrow();
		expect(() => OperationalDesignDomainSchema.parse({ ...baseOdd, weather: [] })).toThrow();
		expect(() => OperationalDesignDomainSchema.parse({ ...baseOdd, vehicleClass: [] })).toThrow();
	});

	it("rejects malformed ISO-3166 codes", () => {
		expect(() => OperationalDesignDomainSchema.parse({ ...baseOdd, region: ["india"] })).toThrow();
		expect(() => OperationalDesignDomainSchema.parse({ ...baseOdd, region: ["in"] })).toThrow();
	});

	it("accepts an optional geofenceId", () => {
		expect(() =>
			OperationalDesignDomainSchema.parse({
				...baseOdd,
				geofenceId: "apcoa-stuttgart-p6",
			}),
		).not.toThrow();
	});
});
...
describe("OperationalContextSchema", () => {
	...
	it("rejects negative speeds", () => {
		expect(() => OperationalContextSchema.parse({ ...baseCtx, proposedSpeedKmh: -1 })).toThrow();
	});
});
...
describe("requireOdd — universal gate", () => {
	...
	it("throws OddViolation with structured reasons on failure", () => {
		try {
			...
			expect(codes).toEqual(["region-out-of-envelope", "speed-exceeds-envelope"]);
		}
	});
