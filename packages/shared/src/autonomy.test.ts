...
describe("resolveAutonomyCapability", () => {
	...
	it("consent off → eligible=false with consent reason", () => {
		const r = resolveAutonomyCapability({
			...baseCtx,
			owner: { ...baseCtx.owner, autonomyConsentGranted: false },
		});
	...
	it("insurance off → eligible=false with insurance reason", () => {
		const r = resolveAutonomyCapability({
			...baseCtx,
			owner: { ...baseCtx.owner, insuranceAllowsAutonomy: false },
		});
		expect(r.eligible).toBe(false);
	...
	it("vehicle year not supported → eligible=false", () => {
		const r = resolveAutonomyCapability({
			...baseCtx,
			vehicle: { ...baseCtx.vehicle, year: 2010 },
		});
		expect(r.eligible).toBe(false);
	...
	it("missing intelligent-park-pilot hw → eligible=false", () => {
		const r = resolveAutonomyCapability({
			...baseCtx,
			vehicle: { ...baseCtx.vehicle, autonomyHw: [] },
		});
		expect(r.eligible).toBe(false);
...
describe("CommandGrantSchema", () => {
	...
	it("rejects grant lifetime exceeding AUTONOMY_MAX_GRANT_SECONDS", () => {
		const tooLong = {
			...
			notBefore: "2026-04-15T10:00:00.000Z",
			notAfter: new Date(
				Date.parse("2026-04-15T10:00:00.000Z") + (AUTONOMY_MAX_GRANT_SECONDS + 60) * 1000,
			).toISOString(),
		};
	...
	it("rejects geofence radius above AUTONOMY_MAX_GEOFENCE_METERS", () => {
		const huge = {
			...
			geofence: {
				lat: 0,
				lng: 0,
				radiusMeters: AUTONOMY_MAX_GEOFENCE_METERS + 1,
			},
		};
	...
	it("rejects notAfter before notBefore", () => {
		const inverted = {
			...validBase,
			notBefore: validBase.notAfter,
			notAfter: validBase.notBefore,
		};
