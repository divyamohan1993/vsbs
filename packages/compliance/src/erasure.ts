...
export const ErasureStatusSchema = z.enum(["pending", "running", "completed", "failed", "held"]);
...
export interface ErasureCoordinator {
	...
	verifyErased(userId: string): Promise<{
		erased: boolean;
		remaining: Partial<Record<SystemName, number>>;
	}>;
...
export class StandardErasureCoordinator implements ErasureCoordinator {
	...
	async requestErasure(input: {
		...
		const receipt: ErasureReceipt = {
			...
			scopes: {},
			holds: [],
			receiptHash: await evidenceHash({
				tombstoneId,
				requestId,
				userId: input.userId,
				scope: input.scope,
	...
	async verifyErased(userId: string): Promise<{
		erased: boolean;
		remaining: Partial<Record<SystemName, number>>;
	}> {
