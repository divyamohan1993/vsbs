...
export const BreachCategorySchema = z.enum(["confidentiality", "integrity", "availability"]);
...
export class StandardBreachReporter implements BreachReporter {
			...
			timeline: [
				{
					at: detectedAt,
					kind: "detected",
					detail: input.description.slice(0, 200),
				},
			],
			notifications: [],
		});
	...
	async notifyDataPrincipals(
		...
		for (const uid of userIds) {
			out.push(
				await this.#notify(incidentId, "principals-inapp", uid, body, "principals-notified"),
			);
		}
		return out;
	}
	...
	async isOverdue(incidentId: string, now: number = Date.now()): Promise<boolean> {
		const inc = this.#mustGet(incidentId);
		return (
			new Date(inc.slaDeadline).getTime() <= now &&
			!inc.notifications.some((n) => n.channel === "dpb-india" && n.status === "sent")
		);
	}
	...
	async #notify(
		...
		inc.timeline = [
			...inc.timeline,
			{ at: notification.sentAt!, kind: timelineKind, detail: recipientHint },
		];
		this.#incidents.set(incidentId, inc);
		return notification;
	}
