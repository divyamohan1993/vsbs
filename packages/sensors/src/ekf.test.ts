...
import { ExtendedKalman, makeCellImbalanceEkf, makeGpsImuEkf, makeSocEkf } from "./fusion.js";
...
describe("makeSocEkf", () => {
	it("tracks coulomb-counting discharge", () => {
		...
		for (let k = 0; k < 600; k++) {
			ekf.predict(dt, [30]);
			const vTerm = 3.2 + 0.6 * (0.8 - (30 * (k + 1)) / (3600 * 60));
			ekf.update([vTerm]);
		}
		expect(ekf.x[0]!).toBeCloseTo(0.8 - 0.0833, 2);
	});
});

describe("makeCellImbalanceEkf", () => {
	it("converges on per-cell voltages", () => {
		const cells = [3.71, 3.7, 3.72, 3.69];
		const ekf = makeCellImbalanceEkf({
			cells,
			processStd: 0.001,
			measStd: 0.005,
		});
