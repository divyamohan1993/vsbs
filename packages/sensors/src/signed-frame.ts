...
export async function generateHmacKey(keyId: string): Promise<SensorFrameKey> {
	const cryptoKey = await crypto.subtle.generateKey({ name: "HMAC", hash: "SHA-256" }, true, [
		"sign",
		"verify",
	]);
	return {
 ...
 */
export async function importHmacKey(keyId: string, rawBytes: Uint8Array): Promise<SensorFrameKey> {
...
export async function verifyFrame(
	...
	if (!key) {
		return {
			ok: false,
			reason: "frame-unknown-key",
			message: `unknown keyId ${f.keyId}`,
		};
	}
	if (key.alg !== f.alg) {
		return {
			ok: false,
			reason: "frame-bad-sig",
			message: `alg mismatch ${key.alg} vs ${f.alg}`,
		};
	}
