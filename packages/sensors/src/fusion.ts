...
import type { FusedObservation, SensorChannel, SensorSample } from "@vsbs/shared";
...
function transpose(m: number[][]): number[][] {
	...
	for (let j = 0; j < cols; j++) {
		...
		for (let i = 0; i < rows; i++) row[i] = m[i]?.[j] ?? 0;
		t.push(row);
	}
	return t;
}
