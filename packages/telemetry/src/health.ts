...
export function makeAlloyDbPing(
	opts: { mode: "sim" | "live"; ping?: () => Promise<void> } & SimOpts,
): CheckFn {
	return async () => {
		if (opts.mode === "sim") {
		...
		if (!opts.ping) {
			return {
				status: "unhealthy",
				latency_ms: 0,
				message: "live mode without ping fn",
			};
		}
		const t = Date.now();
		await opts.ping();
...
export function makeFirestorePing(
	opts: { mode: "sim" | "live"; ping?: () => Promise<void> } & SimOpts,
): CheckFn {
	return async () => {
		if (opts.mode === "sim") {
...
export function makeSecretManagerList(
	opts: { mode: "sim" | "live"; list?: () => Promise<string[]> } & SimOpts,
): CheckFn {
	return async () => {
		if (opts.mode === "sim") {
			return {
				status: "healthy",
				latency_ms: simLatency(opts.jitterMs),
				message: "sim: 0 secrets visible",
			};
		}
		if (!opts.list) {
			return {
				status: "unhealthy",
				latency_ms: 0,
				message: "live mode without list fn",
			};
		}
		const t = Date.now();
...
export function makeLlmProviderPing(
	opts: { mode: "sim" | "live"; ping?: () => Promise<void> } & SimOpts,
): CheckFn {
	return async () => {
		if (opts.mode === "sim") {
