		...
		switch (provider) {
			case "scripted": {
				...
				return new ScriptedProvider({
					...
					model,
					turns: init.turns,
					...(init.loopLastTurn !== undefined ? { loopLastTurn: init.loopLastTurn } : {}),
				});
			}
			...
			case "vertex-gemini": {
				if (!this.#env.GOOGLE_CLOUD_PROJECT || !this.#env.VERTEX_AI_LOCATION) {
					throw new Error(
						"LLM provider vertex-gemini needs GOOGLE_CLOUD_PROJECT and VERTEX_AI_LOCATION",
					);
				}
			...
			case "vertex-claude": {
				if (!this.#env.GOOGLE_CLOUD_PROJECT || !this.#env.VERTEX_AI_LOCATION) {
					throw new Error(
						"LLM provider vertex-claude needs GOOGLE_CLOUD_PROJECT and VERTEX_AI_LOCATION",
					);
				}
