...
describe("parsePinEnvValue", () => {
	...
	it("parses optional capability profile", () => {
		const p = parsePinEnvValue(
			"vertex-claude:claude-haiku-4-5-20251001@2025-10-01#tool-use,low-cost",
		);
...
describe("resolveProfileWithPins", () => {
	...
	it("demo succeeds when every role pin is set", () => {
		const env: Record<string, string> = {};
		for (const role of ALL_ROLES) {
			env[`VSBS_MODEL_PIN_${role.toUpperCase()}`] =
				"google-ai-studio:gemini-2.5-flash-lite@2026-01-01";
		}
